...
import { isValidNamedPackage, isValidMySoObjectId } from '@socialproof/myso/utils';
...
export default async function generate(
	...
	const normalizedPackages =
		packages.length > 0
			? packages.map((p) => {
					...
					if (isValidMySoObjectId(trimmed) || isValidNamedPackage(trimmed)) {
						return {
							network: flags.network ?? 'testnet',
							packageName: isValidMySoObjectId(trimmed) ? trimmed : trimmed.split('/')[1],
							package: trimmed,
							packageId: isValidMySoObjectId(trimmed) ? trimmed : undefined,
						};
					} else {
						return {
	...
	for (const pkg of normalizedPackages) {
		...
		if (isOnChainPackage) {
			...
			const tempDir = mkdtempSync(join(tmpdir(), 'myso-codegen-'));
			console.log(`Generating summary for on-chain package ${packageId} to ${tempDir}`);

			execSync(`myso move summary --package-id ${packageId} --output-directory ${tempDir}`, {
				stdio: 'inherit',
			});
