...
export const onChainPackageSchema = z.object({
	package: z.string().refine((name) => isValidNamedPackage(name) || isValidMySoObjectId(name), {
...
export type MySoCodegenConfig = z.input<typeof configSchema>;
export type ParsedMySoCodegenConfig = z.infer<typeof configSchema>;

export async function loadConfig(): Promise<ParsedMySoCodegenConfig> {
	const config = await cosmiconfig('myso-codegen').search();
