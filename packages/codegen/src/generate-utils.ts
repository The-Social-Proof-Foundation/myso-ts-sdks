...
export interface GetOptions<
	Include extends Omit<MySoClientTypes.ObjectInclude, 'content'> = {},
> extends MySoClientTypes.GetObjectOptions<Include> {
	client: ClientWithCoreApi;
}

export interface GetManyOptions<
	Include extends Omit<MySoClientTypes.ObjectInclude, 'content'> = {},
> extends MySoClientTypes.GetObjectsOptions<Include> {
	client: ClientWithCoreApi;
}
...
export function getPureBcsSchema(typeTag: string | TypeTag): BcsType<any> | null {
	...
	if ('u8' in parsedTag) {
		...
		const pkg = normalizeMySoAddress(structTag.address);
...
export function normalizeMoveArguments(
	...
	for (const [i, argType] of argTypes.entries()) {
		...
		if (argType === '0x3::myso_system::MySoSystemState') {
...
> extends BcsStruct<T, Name> {
	async get<Include extends Omit<MySoClientTypes.ObjectInclude, 'content' | 'json'> = {}>({
		objectId,
		...options
	...
	async getMany<Include extends Omit<MySoClientTypes.ObjectInclude, 'content' | 'json'> = {}>({
		client,
		...options
	...
	> {
		...
		})) as MySoClientTypes.GetObjectsResponse<Include & { content: true }>;
