...
const IMPORT_MAP = {
	Transaction: { module: '@socialproof/myso/transactions', isType: true },
	BcsType: { module: '@socialproof/myso/bcs', isType: true },
	bcs: { module: '@socialproof/myso/bcs', isType: false },
...
export class MoveModuleBuilder extends FileBuilder {
	...
	#getModuleTypeName() {
		...
		if (resolvedAddress === MYSO_FRAMEWORK_ADDRESS) {
			return '0x2';
		} else if (resolvedAddress === MYSO_SYSTEM_ADDRESS) {
			return '0x3';
		} else {
	...
	isContextReference(type: Type): boolean {
		...
		if ('Datatype' in type) {
			return (
				this.#resolveAddress(type.Datatype.module.address) === MYSO_FRAMEWORK_ADDRESS &&
