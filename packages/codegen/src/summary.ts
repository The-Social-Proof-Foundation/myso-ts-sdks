...
export function summaryFromDeserializedModule(mod: DeserializedModule) {
	...
	const summary: ModuleSummary = {
		id: {
			address: normalizeMySoAddress(mod.address_identifiers[moduleHandle.address]),
			name: mod.identifiers[moduleHandle.name],
		},
		immediate_dependencies: mod.module_handles.map((m) => ({
			address: normalizeMySoAddress(mod.address_identifiers[m.address]),
...
function typeFromTypeSignature(mod: DeserializedModule, type: TypeSignature): Type {
	...
	if ('Datatype' in type) {
		...
		return {
			Datatype: {
				module: {
					address: normalizeMySoAddress(mod.address_identifiers[module.address]),
