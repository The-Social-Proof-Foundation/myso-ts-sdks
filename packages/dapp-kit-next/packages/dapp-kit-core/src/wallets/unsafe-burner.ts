...
import { toBase64 } from '@socialproof/utils';
...
export class UnsafeBurnerWallet implements Wallet {
	...
	constructor({ clients }: { clients: ClientWithCoreApi[] }) {
		...
		this.#account = new ReadonlyWalletAccount({
			address: this.#keypair.getPublicKey().toMySoAddress(),
			publicKey: this.#keypair.getPublicKey().toMySoBytes(),
			chains: this.chains,
			features: [MySoSignTransaction, MySoSignAndExecuteTransaction, MySoSignPersonalMessage],
		});
	...
	#signTransaction: MySoSignTransactionMethod = async ({ transaction, signal, chain }) => {
