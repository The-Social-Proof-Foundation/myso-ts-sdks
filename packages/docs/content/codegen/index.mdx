---
title: MySo TypeScript Codegen
---

The `@socialproof/codegen` package automatically generates type-safe TypeScript code from your Move
...
```sh npm2yarn
npm install -D @socialproof/codegen
```
...
Create a `myso-codegen.config.ts` file in your project root:
...
The `MySoCodegenConfig` type supports the following options:
...
| `generateSummaries`            | `boolean`              | `true`  | Automatically run `myso move summary` before generating code. Creates a `package_summaries` directory in your Move package which can be added to `.gitignore`          |
...
Only generate code from specific modules of the MySo framework:
...
- [create-dapp](/dapp-kit/getting-started/create-dapp) - Bootstrap a working dApp with codegen
  already configured
- [MySo Move documentation](https://docs.mysocial.network/concepts/myso-move-concepts)
- [BCS documentation](/bcs)
- [Transaction building](/myso/transaction-building/basics)
- [dApp Kit](/dapp-kit)
