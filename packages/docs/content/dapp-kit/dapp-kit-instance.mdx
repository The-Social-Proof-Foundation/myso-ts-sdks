...
The `createDAppKit` function is the foundation of your MySo dApp. It creates an instance that manages
wallet connections, network configuration, and provides access to the MySo client.
...
import type { UiWallet, UiWalletAccount } from '@socialproof/dapp-kit-core';
...
export const dAppKit = createDAppKit({
	...
	createClient: () => new MySoGrpcClient({ network: 'localnet', baseUrl: 'http://127.0.0.1:9000' }),
...
export const dAppKit = createDAppKit({
	networks: ['mainnet'],
	createClient: () =>
		new MySoGrpcClient({ network: 'mainnet', baseUrl: 'https://fullnode.mainnet.mysocial.network:443' }),
	autoConnect: false,
});
```
