...
<Callout type="info">
	**Migrating from @socialproof/dapp-kit?** If you're currently using the legacy `@socialproof/dapp-kit`
	package, check out our [Migration Guide](/myso/migrations/myso-2.0/dapp-kit) to upgrade to the new
	packages with gRPC and GraphQL support.
</Callout>

The MySo dApp Kit provides tools and components for building decentralized applications on the MySo
...
```sh npm2yarn
npm i @socialproof/dapp-kit-react @socialproof/myso
```

### Vanilla JavaScript and other frameworks

```sh npm2yarn
npm i @socialproof/dapp-kit-core @socialproof/myso
```
...
<Callout type="warning">
	**Deprecated JSON RPC Only**: This legacy package only works with the deprecated JSON RPC API and
	...
	`@socialproof/dapp-kit-core` and `@socialproof/dapp-kit-react`.
</Callout>
