...
The `@socialproof/myso/bcs` package extends `@socialproof/bcs` with MySo specific scheme definitions.

To learn more about using BCS see the [BCS documentation](/bcs).

the `bcs` export of `@socialproof/myso/bcs` contains all the same exports as `bcs` from `@socialproof/bcs` plus
...
The `bcs.Object` schema can be used to serialize and deserialize complete MySo objects:
