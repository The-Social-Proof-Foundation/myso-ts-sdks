...
The Core API is the transport-agnostic interface that all MySo clients implement. It provides a
consistent set of methods for interacting with the MySo blockchain, regardless of whether you're
...
console.log(object.type); // e.g., "0x2::coin::Coin<0x2::myso::MYSO>"
```
...
const result = await client.core.listOwnedObjects({
	owner: '0xabc...',
	filter: {
		StructType: '0x2::coin::Coin<0x2::myso::MYSO>',
	},
	limit: 10,
});
...
const balance = await client.core.getBalance({
	owner: '0xabc...',
	coinType: '0x2::myso::MYSO', // Optional, defaults to MYSO
});
...
Resolve an address to its default MySoNS name.
...
console.log(name); // e.g., "example.myso"
```
...
const { type } = await client.core.mvr.resolveType({
	type: '@socialproof/myso::coin::Coin<@socialproof/myso::myso::MYSO>',
});

console.log(type); // "0x2::coin::Coin<0x2::myso::MYSO>"
```
...
## MySoClientTypes Namespace

The `MySoClientTypes` namespace contains all type definitions for the Core API. Import it when you
...
function processObject(obj: MySoClientTypes.Object<{ content: true }>) {
...
async function fetchBalance(
	...
	owner: string,
): Promise<MySoClientTypes.CoinBalance> {
	const { balance } = await client.core.getBalance({ owner });
	return balance;
}

// Type options
const options: MySoClientTypes.GetObjectOptions<{ content: true }> = {
	objectId: '0x123...',
	include: { content: true },
};
```
