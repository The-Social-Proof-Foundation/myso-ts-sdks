---
title: MySoGrpcClient
---

The `MySoGrpcClient` provides access to the Full Node gRPC API.

For more complete details on what is available through this API see the
[gRPC API docs](https://docs.mysocial.network/concepts/data-access/grpc-overview).

## Creating a gRPC client

To get started, create a `MySoGrpcClient` instance by specifying a network and base URL:
...
The `MySoGrpcClient` exposes several service clients for lower-level access to the gRPC API. These
service clients are generated using [protobuf-ts](https://github.com/timostamm/protobuf-ts), which
provides type-safe gRPC clients for TypeScript. For more details on how to use gRPC with MySo, see
the [gRPC overview](https://docs.mysocial.network/concepts/data-access/grpc-overview).
