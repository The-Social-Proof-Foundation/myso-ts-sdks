---
title: MySoJsonRpcClient
---

<Callout type="warn">
	The MySo JSON-RPC API has been deprecated. We recommend migration to
	[MySoGrpcClient](/myso/clients/grpc) or [MySoGraphQLClient](/myso/clients/graphql) as soon as
	possible.
</Callout>

The `MySoJsonRpcClient` connects to a MySo network's JSON-RPC server. It implements the
[Core API](/myso/clients/core), so it can be used with any SDK that accepts `ClientWithCoreApi`.
...
## Connecting to a MySo network

To establish a connection to a network, import `MySoJsonRpcClient` from `@socialproof/myso/client` and pass
...
const client = new MySoJsonRpcClient({ url: rpcUrl, network: 'devnet' });
...
For local development, you can run `cargo run --bin myso -- start --with-faucet --force-regenesis` to
spin up a local network with a local validator, a Full node, and a faucet server. Refer to
[the Local Network guide](https://docs.mysocial.network/guides/developer/getting-started/local-network) for
more information.

## Manually calling unsupported RPC methods

You can use `MySoJsonRpcClient` to call any RPC method the node you're connecting to exposes. Most
RPC methods are built into `MySoJsonRpcClient`, but you can use `call` to leverage any methods
...
// asynchronously call mysox_getCommitteeInfo
const committeeInfo = await client.call('mysox_getCommitteeInfo', []);
```

For a full list of available RPC methods, see the
[RPC documentation](https://docs.mysocial.network/references/myso-api).

## Customizing the transport

The `MySoJsonRpcClient` uses a `Transport` class to manage connections to the RPC node. The default
`MySoHTTPTransport` (alias for `JsonRpcHTTPTransport`) makes both JSON RPC requests, as well as
...
```typescript
import { MySoJsonRpcClient, JsonRpcHTTPTransport } from '@socialproof/myso/jsonRpc';
...
const client = new MySoJsonRpcClient({
	transport: new JsonRpcHTTPTransport({
		...
		websocket: {
			...
			url: 'wss://fullnode.devnet.mysocial.network:443',
		},
		rpc: {
			headers: {
...
`MySoJsonRpcClient` exposes a number of RPC methods that return paginated results. These methods
...
In addition to the RPC methods mentioned above, `MySoJsonRpcClient` also exposes some methods for
