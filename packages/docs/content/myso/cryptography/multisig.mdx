...
The MySo TypeScript SDK provides a `MultiSigPublicKey` class to support
[Multi-Signature](https://docs.mysocial.network/concepts/cryptography/transaction-auth/multisig) (MultiSig)
...
transaction is equal to or greater than the threshold value, then the MySo network considers the
transdaction valid.
