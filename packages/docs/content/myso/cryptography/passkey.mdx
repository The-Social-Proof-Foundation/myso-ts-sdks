...
Similar to other Keypair classes, the MySo TypeScript SDK provides the `PasskeyKeypair` that handles
communication with the passkey device and signing with it. This SDK defines the address and
signatures according to [SIP-9](https://github.com/myso-foundation/sips/blob/main/sips/sip-9.md).

To use, import the `PasskeyKeypair` class from `@socialproof/myso/keypairs/passkey`.
...
const keypair = await PasskeyKeypair.getPasskeyInstance(
	new BrowserPasskeyProvider('MySo Passkey Example', {
...
MySo supports passkey wallets on any device that complies with the WebAuthn standard. This includes
