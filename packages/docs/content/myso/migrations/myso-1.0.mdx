...
- [A new GraphQL Client for the newly released MySo GraphQL API](/myso/clients/graphql)
...
The majority of changes in this release are in the `@socialproof/myso` package, but the changes to API
...
## Changes to `@socialproof/myso`

As part of the 1.0 release the `@socialproof/myso.js` package has been renamed to `@socialproof/myso`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @socialproof/myso.js
npm install @socialproof/myso
```

Then update all imports to use the new package name:

```diff
- import { MySoJsonRpcClient } from '@socialproof/myso.js'
+ import { MySoJsonRpcClient } from '@socialproof/myso'
```
...
coming to other parts of the MYSO ecosystem in the future.
...
- `mysoClient.waitForTransactionBlock({ transactionBlock })` ->
  `mysoClient.waitForTransaction({ transaction })`
- `mysoClient.signAndExecuteTransactionBlock({ transactionBlock })` ->
  `mysoClient.signAndExecuteTransaction({ transaction })`

There are a few methods on MySoJsonRpcClient that retain the `TransactionBlock` name because they
correspond to specific JSON RPC calls. The following methods are unchanged:

- `mysoClient.getTransactionBlock`
- `mysoClient.queryTransactionBlocks`
- `mysoClient.executeTransactionBlock`
- `mysoClient.dryRunTransactionBlock`
- `mysoClient.devInspectTransactionBlock`
...
```diff
- import { Transactions } from '@socialproof/myso/transactions'
...
- [`TransactionObjectArgument`](/typedoc/types/_mysten_myso.transactions.TransactionObjectArgument.html)
  shape changed
- [`TransactionResult`](/typedoc/types/_mysten_myso.transactions.TransactionResult.html) shape
  changed
- [`TransactionObjectInput`](/typedoc/types/_mysten_myso.transactions.TransactionObjectInput.html)
  shape changed
- [`TransactionArgument`](/typedoc/types/_mysten_myso.transactions.TransactionArgument.html) shape
  changed
- [`TransactionBlockInput`](/typedoc/types/_mysten_myso.transactions.TransactionBlockInput.html)
  shape changed
...
The `publicKeyFromMySoBytes` has been moved to `@socialproof/myso/verify`

```diff
- import { publicKeyFromMySoBytes } from '@socialproof/myso/multisig'
...
`mysoClient.executeTransactionBlock` for returning additional data.
...
`showRawEffects` to true if using `mysoClient.executeTransactionBlock`, or by returning an object
...
[wallet-standard documentation](https://docs.mysocial.network/standards/wallet-standard).

The old `myso:signTransactionBlock` and `myso:signAndExecuteTransactionBlock` methods have been
deprecated in favor of the new `myso:signTransaction` and `myso:signAndExecuteTransaction` methods,
