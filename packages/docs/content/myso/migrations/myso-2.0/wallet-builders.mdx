...
`@socialproof/wallet-standard` interface.

## Key Changes

### Removal of `myso:reportTransactionEffects`

The `myso:reportTransactionEffects` feature has been removed entirely. If your wallet implements this
feature, remove it.
...
effects when using the new MySo client APIs.
...
-	).sign({ client: mysoClient, signer: keypair });
-
...
- Use `signer.signAndExecuteTransaction()` instead of `mysoClient.executeTransactionBlock()`
