...
This guide covers recommended patterns for building TypeScript SDKs that integrate with the MySo SDK.
...
transport-agnostic interface implemented by all MySo clients. This ensures your SDK works with any
...
SDKs should declare all `@socialproof/*` packages as **peer dependencies** rather than direct
...
extend the MySo client with custom functionality. This makes it easier to use custom SDKs across the
ecosystem without having to build custom bindings (like react context providers) for each individual
SDK and client.

### Extension Pattern

Client extensions use the `$extend` method to add functionality to any MySo client. Create a factory
...
- **[@socialproof/file-storage](https://www.npmjs.com/package/@socialproof/file-storage)** - Decentralized storage
- **[@socialproof/mydata](https://www.npmjs.com/package/@socialproof/mydata)** - Encryption and key management
...
For SDKs that interact with Move contracts, use **[@socialproof/codegen](/codegen)** to generate
