---
title: MySo Programmable Transaction Basics
---

This example starts by constructing a transaction to send MYSO. To construct transactions, import the
...
tx.transferObjects([coin], '0xSomeMySoAddress');
```
...
[the MySoJsonRpcClient docs for more details](/myso/clients/json-rpc#arguments)). These options will
...
inputs, and produces results. The inputs for a transaction depend on the kind of command. MySo
...
tx.transferObjects([coin], tx.pure(bcs.Address.serialize('0xSomeMySoAddress')));
```
...
When building a transaction, MySo expects all objects to be fully resolved, including the object
...
In most cases, building requires your MySoJsonRpcClient to fully resolve input values.
