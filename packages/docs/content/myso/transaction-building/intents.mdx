...
The `CoinWithBalance` intent makes it easy to get a coin with a specific balance. For MYSO, this has
...
This approach works well for MYSO, but can't be used for other coin types. The CoinWithBalance intent
...
tx.setSender(keypair.toMySoAddress());

tx.transferObjects(
	[
		// Create a MYSO coin (balance is in MIST)
...
transfer MYSO that does not use the gas coin, you can set the `useGasCoin` option to `false`:
...
coinWithBalance intent may use all the MYSO coins, leaving no coins to use for gas.
