...
This guide will help you set up and start using the Payment Kit SDK in your MySo application.

## Prerequisites

Before you begin, ensure you have:

- A MySo wallet with testnet/mainnet MYSO tokens
- Basic understanding of TypeScript and the MySo blockchain

## Installation

Install the Payment Kit SDK and the MySo TypeScript SDK:
...
The Payment Kit SDK extends the standard `MySoGrpcClient` to provide payment functionality. Here's
how to set it up:
...
// Create a MySo client with Payment Kit extension
...
const tx = client.paymentKit.tx.processRegistryPayment({
	...
	coinType: '0x2::myso::MYSO', // Coin type (MYSO in this case)
	amount: 1n * MIST_PER_MYSO, // 1 MYSO (in MIST)
	receiver,
	sender: sender,
});
...
2. **Coin Type**: The type of coin being transferred (MYSO token)
3. **Amount**: The payment amount in the smallest unit (MIST for MYSO)
...
Payment Kit supports any MySo coin type. Here's how to process payments with custom coins:
...
```ts
import { createPaymentTransactionUri } from '@socialproof/payment-kit';

// Create a payment URI
const paymentUri = createPaymentTransactionUri({
	receiverAddress: '0x123...abc',
	amount: 1000000000n, // 1 MYSO in MIST
	coinType: '0x2::myso::MYSO',
	nonce: crypto.randomUUID(),
...
// Output: myso:pay?receiver=0x123...abc&amount=1000000000&coinType=0x2%3A%3Amyso%3A%3AMYSO&nonce=...&label=Coffee%20Payment&message=Payment%20for%20espresso&registry=my-registry
```

The URI parameters include:

- **receiver**: The MySo address receiving the payment (required)
...
```ts
import { parsePaymentTransactionUri } from '@socialproof/payment-kit';
...
- [Payment Processing](./payment-processing) - MySo dive into payment models
