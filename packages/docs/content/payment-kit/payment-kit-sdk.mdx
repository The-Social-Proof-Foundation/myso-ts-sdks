...
Function to create a Payment Kit client extension for `MySoGrpcClient`.

```ts
paymentKit<Name extends string = 'paymentKit'>({
	name = 'paymentKit' as Name,
}): MySoClientRegistration<PaymentKitCompatibleClient, Name, PaymentKitClient>
```
...
type ProcessRegistryPaymentOptions = {
	...
	coinType: string; // Coin type (e.g., '0x2::myso::MYSO')
...
follow the `myso:pay` protocol and can be used to encode payment parameters for QR codes, deep links,
or other sharing mechanisms.

### URI Format

Payment URIs follow the `myso:pay` protocol:
...
| ---------- | -------------------------------------------- |
| `receiver` | Recipient MySo address                        |
| `amount`   | Payment amount in smallest unit (e.g., MIST) |
| `coinType` | Coin type (e.g., `0x2::myso::MYSO`)            |
...
type PaymentUriParams = {
	receiverAddress: string; // Recipient MySo address
...
- `PaymentKitUriError` if registry ID is not a valid MySo object ID
...
const uri =
	'myso:pay?receiver=0x1234...&amount=1000000000&coinType=0x2::myso::MYSO&nonce=abc123&registry=my-registry';
...
console.log(params.coinType); // '0x2::myso::MYSO'
...
- `PaymentKitUriError` if URI doesn't start with `myso:pay?`
