...
MySo linking allows external applications, websites, and services to open Slush directly to specific
...
<Callout type="warning">
	MySo linking is not currently supported for the Slush browser extension.
</Callout>
...
```
slush://swap?fromCoinType=0x2::myso::MYSO
...
| Parameter      | Type   | Required | Description                                 |
| -------------- | ------ | -------- | ------------------------------------------- |
| `fromCoinType` | string | No       | Source token type address (defaults to MYSO) |
...
```
# Open swap with MYSO as source
slush://swap

# Pre-fill swap from MYSO to USDC
slush://swap?fromCoinType=0x2::myso::MYSO&toCoinType=0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7::usdc::USDC

# Pre-fill swap amount
slush://swap?fromCoinType=0x2::myso::MYSO&presetAmount=100
```
...
```
# View MYSO token details
slush://tokens?coinType=0x2::myso::MYSO
...
slush://mysome?action=RECEIVE

# Open QR code scanner
slush://mysome?action=SCAN_QR_CODE
```
...
| `staking/native-stake`   | Native MYSO staking     |
| `staking/native-unstake` | Unstake native MYSO     |
...
Linking.openURL('slush://swap?fromCoinType=0x2::myso::MYSO');
