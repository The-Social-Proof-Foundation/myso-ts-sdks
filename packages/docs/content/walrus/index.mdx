...
The `file-storageFile` API provides a higher level abstraction so that applications don't need to worry
...
and will return a `file-storageFile`.
...
```ts
const [file1, file2] = await client.file-storage.getFiles({ ids: [anyBlobId, orQuiltId] });
```

A `file-storageFile` works like a `Response` object from the `fetch` API:
...
A `file-storageFile` may also have and `identifier` and `tags` properties if the file was stored in a
quilt.
...
You can also get a `file-storageBlob` instead of a `file-storageFile` if you have the blobId:

```ts
const blob = await client.file-storage.getBlob({ blobId });
```
...
You can also construct a `file-storageFile` from a `Uint8Array`, `Blob`, or a `string` which can then be
stored on file-storage:

```ts
const file1 = file-storageFile.from({
	contents: new Uint8Array([1, 2, 3]),
	identifier: 'file1.bin',
});
const file2 = file-storageFile.from({
	contents: new Blob([new Uint8Array([1, 2, 3])]),
	identifier: 'file2.bin',
});
const file3 = file-storageFile.from({
...
}[] = await client.file-storage.writeFiles({
...
const flow = client.file-storage.writeFilesFlow({
	files: [
...
(paid in MIST) and the `file-storageClient` will automatically determine the required tip for your upload
...
In case you do not want to use the `file-storageFile` abstractions, you can use the `readBlob` and
...
```ts
const blob = await client.file-storage.readBlob({ blobId });
```

### Writing Blobs

The writeBlob method can be used to write a blob (as a `Uint8Array`) to file-storage. You will need to
specify how long the blob should be stored for, and if the blob should be deletable.

```ts
const file = new TextEncoder().encode('Hello from the TS SDK!!!\n');

const { blobId } = await client.file-storage.writeBlob({
	blob: file,
	deletable: false,
	epochs: 3,
	signer: keypair,
...
For a complete overview of the available methods on the `file-storageClient` you can reference type
[TypeDocs](/typedoc/classes/_mysten_file-storage.file-storageClient.html)
...
changes there may be times when the data cached in the `file-storageClient` can become invalid. Errors
that result from this situation will extend the `Retryablefile-storageClientError` class.

You can check for these errors, and reset the client before retrying:

```ts
import { Retryablefile-storageClientError } from '@socialproof/file-storage';

if (error instanceof Retryablefile-storageClientError) {
	client.file-storage.reset();

	/* retry your operation */
}
```

`Retryablefile-storageClientError` are not guaranteed to succeed after resetting the client and retrying,
...
File Storage is designed to handle some nodes being down, and the SDK will only throw errors when it can't
...
```ts
import file-storageWasmUrl from '@socialproof/file-storage-wasm/web/file_storage_wasm_bg.wasm?url';
