...
export class EnokiWallet implements Wallet {
	...
	#getCurrentNetwork: () => MySoClientTypes.Network;
	...
	#signTransaction: MySoSignTransactionMethod = async ({ transaction, chain, account, signal }) => {
		...
		const mysoAddress = keypair.toMySoAddress();

		if (mysoAddress !== account.address) {
			throw new Error(
				`The specified account ${account.address} does not match the currently connected Enoki address ${mysoAddress}.`,
			);
		}

		parsedTransaction.setSenderIfNotSet(mysoAddress);
		...
		if (zkLoginState) {
			return [
				new ReadonlyWalletAccount({
					...
					chains: this.chains,
					icon: this.icon,
					features: [MySoSignPersonalMessage, MySoSignTransaction, MySoSignAndExecuteTransaction],
	...
	async #getSignerContext(chain?: IdentifierString) {
		...
		if (!sessionContext) {
			throw new Error(
				`A valid MySo chain identifier was not provided in the request. Please report this issue to the dApp developer. Examples of valid MySo chain identifiers are 'myso:testnet' and 'myso:mainnet'. Consider using the '@socialproof/dapp-kit' package, which provides this value automatically.`,
			);
		}
	...
	async #createSession({ network }: { network: MySoClientTypes.Network }) {
	...
	async #createAuthorizationURL(sessionContext: EnokiSessionContext, pkceContext?: PKCEContext) {
		...
		switch (this.#provider) {
			...
			case 'twitch':
				...
				break;
			case 'onefc':
				oauthUrl = `https://login.onepassport.onefc.com/de3ee5c1-5644-4113-922d-e8336569a462/b2c_1a_prod_signupsignin_onemysozklogin/oauth2/v2.0/authorize?${params}`;
				break;
			case 'playtron':
