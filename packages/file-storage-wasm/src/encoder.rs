...
use file-storage_core::encoding::{
    EncodingConfig, EncodingConfigEnum, EncodingFactory, Primary, SliverData,
};
use file-storage_core::metadata::{BlobMetadata, BlobMetadataApi};
use file-storage_core::{BlobId, EncodingType};
...
impl BlobEncoder {
    ...
    fn write_sliver_data_bcs<T: file-storage_core::encoding::EncodingAxis>(
        sliver: &SliverData<T>,
        buffer: &Uint8Array,
    ) -> Result<(), JsError> {
