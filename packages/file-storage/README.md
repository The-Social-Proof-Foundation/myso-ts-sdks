...
```bash
npm install --save @socialproof/file-storage @socialproof/myso
```

## Setup

To use the file-storage SDK you will need to create a Client from the typescript SDK, and extend it with
the file-storage SDK.
...
The file-storage SDK currently includes all the relevant package and object IDs needed for connecting to
testnet. You can also manually configure the file-storage sdk to use a different set of ids, allowing you
to connect to a different network or updated deployment of the file-storage contracts.
...
The File Storage TS SDK is designed to work directly with file-storage storage nodes, or with the file-storage upload
relay. When using the file-storage SDK without an upload relay, it is important to understand that reading
and writing file-storage blobs requires a lot of requests (~2200 to write a blob, ~335 to read a blob).
The upload relay will reduce the number of requests needed to write a blob, but reads through the
file-storage SDK will still require a lot of requests. For many applications, using publishers and
aggregators is recommended, but the TS SDK can be useful when building applications where the
application needs to directly interact with file-storage or users need to pay for their own storage
directly.

The file-storage sdk exposes high level methods for reading and writing blobs, as well as lower level
...
The `FileStorageFile` API provides a higher level abstraction so that applications don't need to worry
about how data is stored in file-storage. Today it handles data stored directly in blobs, and data stored
...
and will return a `FileStorageFile`.
...
```ts
const [file1, file2] = await client.fileStorage.getFiles({ ids: [anyBlobId, orQuiltId] });
```

A `FileStorageFile` works like a `Response` object from the `fetch` API:
...
A `FileStorageFile` may also have and `identifier` and `tags` properties if the file was stored in a
quilt.
...
You can also get a `FileStorageBlob` instead of a `FileStorageFile` if you have the blobId:

```ts
const blob = await client.fileStorage.getBlob({ blobId });
```
...
// Get files by tag
const textFiles: FileStorageFile[] = await blob.files({ tags: [{ 'content-type': 'text/plain' }] });
...
You can also construct a `FileStorageFile` from a `Uint8Array`, `Blob`, or a `string` which can then be
stored on file-storage:

```ts
const file1 = FileStorageFile.from({
	contents: new Uint8Array([1, 2, 3]),
	identifier: 'file1.bin',
});
const file2 = FileStorageFile.from({
	contents: new Blob([new Uint8Array([1, 2, 3])]),
	identifier: 'file2.bin',
});
const file3 = FileStorageFile.from({
...
Once you have your files you can use the `writeFiles` method to write them to file-storage.

Along with the files, you will also need to provide a `Signer` instance that signs and pays for the
transaction/storage fees. The signer's address will need to have sufficient `MYSO` to cover the
...
const flow = client.fileStorage.writeFilesFlow({
	files: [
...
To use an upload relay, you can add the `uploadRelay` option when adding the file-storage extension:
...
(paid in MIST) and the `FileStorageClient` will automatically determine the required tip for your upload
...
`https://upload-relay.testnet.file-storage.space/v1/tip-config`) and may either be a `const` or a `linear`
tip.
...
In case you do not want to use the `FileStorageFile` abstractions, you can use the `readBlob` and
...
```ts
const blob = await client.fileStorage.readBlob({ blobId });
```

### Writing Blobs

Thw writeBlob method can be used to write a blob (as a `Uint8Array`) to file-storage. You will need to
specify how long the blob should be stored for, and if the blob should be deletable.

```ts
const file = new TextEncoder().encode('Hello from the TS SDK!!!\n');

const { blobId } = await client.fileStorage.writeBlob({
	blob: file,
	deletable: false,
	epochs: 3,
	signer: keypair,
...
For a complete overview of the available methods on the `FileStorageClient` you can reference type
[TypeDocs](http://sdk.mystenlabs.com/typedoc/classes/_mysten_file-storage.FileStorageClient.html)

## Examples

There are a number of simple
[examples you can reference](https://github.com/MystenLabs/ts-sdks/tree/main/packages/file-storage/examples)
in the `ts-sdks` repo that show things like building simple aggregators and publishers with the
file-storage SDK

## Error handling

The SDK exports all the error classes for different types of errors that can be thrown. File Storage is a
fault tolerant distributed system, where many types of errors can be recovered from. During epoch
changes there may be times when the data cached in the `FileStorageClient` can become invalid. Errors
that result from this situation will extend the `RetryableFileStorageClientError` class.

You can check for these errors, and reset the client before retrying:

```ts
import { RetryableFileStorageClientError } from '@socialproof/file-storage';

if (error instanceof RetryableFileStorageClientError) {
	client.fileStorage.reset();

	/* retry your operation */
}
```

`RetryableFileStorageClientError` are not guaranteed to succeed after resetting the client and retrying,
...
File Storage is designed to be handle some nodes being down, and the SDK will only throw errors when it
...
Reading and writing blobs directly from storage nodes requires a lot of requests. The file-storage SDK
...
The file-storage SDK requires wasm bindings to encode and decode blobs. When running in node or bun and
...
and then passed into the file-storage client:

```ts
import fileStorageWasmUrl from '@socialproof/file-storage-wasm/web/file_storage_wasm_bg.wasm?url';
...
const client = new MySoGrpcClient({
	...
	fileStorage({
		wasmUrl: 'https://unpkg.com/@socialproof/file-storage-wasm@latest/web/file_storage_wasm_bg.wasm',
	}),
);
```

In next.js when using file-storage in API routes, you may need to tell next.js to skip bundling for the
file-storage packages:

```ts
// next.config.ts
const nextConfig: NextConfig = {
	serverExternalPackages: ['@socialproof/file-storage', '@socialproof/file-storage-wasm'],
};
```
