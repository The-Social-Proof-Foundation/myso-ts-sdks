...
export function useBenchmark() {
	...
	useEffect(() => {
		const savedResults = localStorage.getItem('file-storage-benchmark-results');
		if (savedResults) {
			try {
	...
	const removeResult = (id: string) => {
		...
		localStorage.setItem('file-storage-benchmark-results', JSON.stringify(newResults));
	};

	const clearResults = () => {
		setResults([]);
		localStorage.removeItem('file-storage-benchmark-results');
	};
		...
		async (settings: BenchmarkSettings, onError: (error: string) => void) => {
			...
			try {
				...
				for (let iteration = 1; iteration <= settings.iterations; iteration++) {
					...
					const fileStorageClient = settings.useUploadRelay
						? mysoClient.fileStorageWithRelay
						: mysoClient.fileStorageWithoutRelay;
					const flow = fileStorageClient.writeBlobFlow({ blob: data });
					...
					registerTx.setSender(keypair.toMySoAddress());
					...
					certifyTx.setSender(keypair.toMySoAddress());
				...
				setResults((prevResults) => {
					...
					localStorage.setItem('file-storage-benchmark-results', JSON.stringify(updatedResults));
					return updatedResults;
				});
