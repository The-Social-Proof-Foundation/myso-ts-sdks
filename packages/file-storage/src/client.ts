...
import type { ClientCache, ClientWithCoreApi } from '@socialproof/myso/client';
import type { TransactionObjectArgument, TransactionResult } from '@socialproof/myso/transactions';
import { coinWithBalance, Transaction } from '@socialproof/myso/transactions';
import { normalizeStructTag, parseStructTag } from '@socialproof/myso/utils';

import {
	MAINNET_FILE_STORAGE_PACKAGE_CONFIG,
	statusLifecycleRank,
	TESTNET_FILE_STORAGE_PACKAGE_CONFIG,
} from './constants.js';
import {
	addMetadata,
	Blob,
	...
	removeMetadataPair,
} from './contracts/file-storage/blob.js';
import type { Committee } from './contracts/file-storage/committee.js';
import * as metadata from './contracts/file-storage/metadata.js';
import { StakingInnerV1 } from './contracts/file-storage/staking_inner.js';
import { StakingPool } from './contracts/file-storage/staking_pool.js';
import { Staking } from './contracts/file-storage/staking.js';
import { Storage } from './contracts/file-storage/storage_resource.js';
import { SystemStateInnerV1 } from './contracts/file-storage/system_state_inner.js';
import {
	certifyBlob,
	deleteBlob,
	extendBlob,
	...
	reserveSpace,
	System,
} from './contracts/file-storage/system.js';
import {
	...
	RetryableFileStorageClientError,
...
import { MySoObjectDataLoader } from './utils/object-loader.js';
...
import { encodeQuilt, encodeQuiltPatchId, parseFileStorageId } from './utils/quilts.js';
import { BlobReader } from './files/readers/blob.js';
import { FileStorageBlob } from './files/blob.js';
import { FileStorageFile } from './files/file.js';
import { QuiltFileReader } from './files/readers/quilt-file.js';
import { QuiltReader } from './files/readers/quilt.js';
import { retry } from './utils/retry.js';

export function fileStorage<const Name = 'fileStorage'>({
	packageConfig,
	name = 'fileStorage' as Name,
	...options
}: FileStorageOptions<Name> = {}) {
	return {
		name,
		register: (client: ClientWithCoreApi) => {
			const fileStorageNetwork = client.network;

			if (fileStorageNetwork !== 'mainnet' && fileStorageNetwork !== 'testnet') {
				throw new FileStorageClientError('File Storage client only supports mainnet and testnet');
			}
			...
			return new FileStorageClient(
				packageConfig
					...
					: {
							network: fileStorageNetwork as 'mainnet' | 'testnet',
							mysoClient: client,
							...options,
						},
			);
...
export class FileStorageClient {
	...
	#objectLoader: MySoObjectDataLoader;
	...
	constructor(config: FileStorageClientConfig) {
		if (config.network && !config.packageConfig) {
			...
			switch (network) {
				case 'testnet':
					this.#packageConfig = TESTNET_FILE_STORAGE_PACKAGE_CONFIG;
					break;
				case 'mainnet':
					this.#packageConfig = MAINNET_FILE_STORAGE_PACKAGE_CONFIG;
					break;
				default:
					throw new FileStorageClientError(`Unsupported network: ${network}`);
			}
		} else {
		...
		this.#objectLoader = new MySoObjectDataLoader(this.#mysoClient);
		this.#cache = this.#mysoClient.cache.scope('@socialproof/file-storage');
	}

	/** The Move type for a WAL coin */
	#walType() {
		return this.#cache.read(['walType'], async () => {
			const stakeWithPool = await this.#mysoClient.core.getMoveFunction({
			...
			if (toStakeCoinType?.$kind !== 'datatype') {
				throw new FileStorageClientError('WAL type not found');
			}
	...
	/** The cached system object for the file-storage package */
	systemObject() {
		return this.#objectLoader.load(this.#packageConfig.systemObjectId, System);
	}

	/** The cached staking pool object for the file-storage package */
	stakingObject() {
		return this.#objectLoader.load(this.#packageConfig.stakingPoolId, Staking);
	}

	/** The system state for the current version of file-storage contract  */
	async systemState() {
	...
	/** The staking state for the current version of file-storage contract */
	async stakingState() {
	...
	createStorage({ size, epochs, walCoin }: StorageWithSizeOptions) {
		return async (tx: Transaction) => {
			...
			const systemState = await this.systemState();
			const encodedSize = encodedBlobLength(size, systemState.committee.n_shards);
			const [{ storageCost }, fileStoragePackageId] = await Promise.all([
				this.storageCost(size, epochs),
				this.#getFileStoragePackageId(),
			]);
	 ...
	 * @example
	 * ```ts
	 * const tx = client.createStorageTransaction({ size: 1000, epochs: 3, owner: signer.toMySoAddress() });
	 * ```
	 */
	...
	async executeCreateStorageTransaction({
		...
		const transaction = this.createStorageTransaction({
			...options,
			owner: options.transaction?.getData().sender ?? signer.toMySoAddress(),
		});
		...
		const createdObjects = await this.#mysoClient.core.getObjects({
			objectIds: createdObjectIds,
			include: { content: true },
		});

		const mysoBlobObject = createdObjects.objects.find(
			(object) => !(object instanceof Error) && object.type === blobType,
		);

		if (mysoBlobObject instanceof Error || !mysoBlobObject) {
			throw new FileStorageClientError(
		...
		return {
			digest,
			storage: Storage.parse(mysoBlobObject.content),
		};
	}
	...
	registerBlob({
		...
		return async (tx: Transaction) => {
			...
			const fileStoragePackageId = await this.#getFileStoragePackageId();
	...
	async executeRegisterBlobTransaction({
		...
		const transaction = this.registerBlobTransaction({
			...options,
			owner: options.owner ?? options.transaction?.getData().sender ?? signer.toMySoAddress(),
		});
		const blobType = await this.getBlobType();
		const { digest, effects } = await this.#executeTransaction(
			transaction,
		...
		return {
			digest,
			blob: Blob.parse(mysoBlobObject.content),
		};
	}

	async #getCreatedBlob(digest: string) {
		const blobType = await this.getBlobType();
		const result = await this.#mysoClient.core.waitForTransaction({
			digest,
		...
		return Blob.parse(mysoBlobObject.content);
	}
	...
	async executeDeleteBlobTransaction({
		...
		const { digest } = await this.#executeTransaction(
			this.deleteBlobTransaction({
				blobObjectId,
				transaction,
				owner: transaction.getData().sender ?? signer.toMySoAddress(),
			}),
			signer,
			'delete blob',
		);
	...
	async readBlobAttributes({
		...
		const response = await this.#mysoClient.core.getDynamicField({
	...
	async writeBlobToUploadRelay(options: WriteBlobToUploadRelayOptions): Promise<{
		...
		if (!this.#uploadRelayClient) {
			throw new FileStorageClientError('Upload relay not configured');
		}
	...
	async writeBlob({
		...
		if (!this.#uploadRelayConfig) {
			...
			const mysoBlobObject = await this.executeRegisterBlobTransaction({
				signer,
				size: blob.length,
				epochs,
				blobId,
				rootHash,
				deletable,
				owner: owner ?? signer.toMySoAddress(),
				attributes,
			});

			const blobObjectId = mysoBlobObject.blob.id;
			...
			await this.#mysoClient.core.waitForTransaction({
	...
	async #getNodeByShardIndex(committeeInfo: CommitteeInfo, index: number) {
		...
		if (!node) {
			throw new FileStorageClientError(`Node for shard index ${index} not found`);
		}
		return node;
	}
	...
	async getFiles({ ids }: { ids: string[] }) {
		...
		const parsedIds = ids.map((id) => parseFileStorageId(id));
