...
export function mydata<Name = 'mydata'>({ name = 'mydata' as Name, ...options }: MyDataOptions<Name>) {
	return {
		name,
		register: (client: MyDataCompatibleClient) => {
...
export class MyDataClient {
	...
	constructor(options: MyDataClientOptions) {
		this.#mysoClient = options.mysoClient;
	...
	async encrypt({
		...
		const packageObj = await this.#mysoClient.core.getObject({ objectId: packageId });
	 ...
	 * @param txBytes - The transaction bytes to use (that calls mydata_approve* functions).
