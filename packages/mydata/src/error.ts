...
export class UserError extends MyDataError {}

// Errors returned by the MyData server
export class MyDataAPIError extends MyDataError {
	constructor(
		message: string,
	...
	static async assertResponse(response: Response, requestId: string) {
		...
		let errorInstance: MyDataAPIError;
		try {
			const text = await response.text();
			const error = JSON.parse(text)['error'];
			const message = JSON.parse(text)['message'];
			errorInstance = MyDataAPIError.#generate(error, message, requestId);
		} catch {
...
// Errors returned by the MyData server that indicate that the PTB is invalid

export class InvalidPTBError extends MyDataAPIError {
	constructor(requestId?: string, message?: string) {
		super('PTB does not conform to the expected format ' + message, requestId);
	}
}

export class InvalidPackageError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('Package ID used in PTB is invalid', requestId);
	}
}

export class InvalidParameterError extends MyDataAPIError {
	constructor(requestId?: string) {
		super(
...
// Errors returned by the MyData server that indicate that the user's signature is invalid

export class InvalidUserSignatureError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('User signature on the session key is invalid', requestId);
	}
}

export class InvalidSessionKeySignatureError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('Session key signature is invalid', requestId);
	}
}

export class InvalidMVRNameError extends MyDataAPIError {
	constructor(requestId?: string) {
...
export class InvalidKeyServerObjectIdError extends MyDataAPIError {
	constructor(requestId?: string) {
...
export class UnsupportedPackageIdError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('Requested package is not supported', requestId);
	}
}

// Errors returned by the MyData server that indicate that the SDK version is invalid (implying that HTTP headers used by the SDK are being removed) or deprecated (implying that the SDK should be upgraded).

export class InvalidSDKVersionError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('SDK version is invalid', requestId);
	}
}

export class InvalidSDKTypeError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('SDK type is invalid', requestId);
	}
}

export class DeprecatedSDKVersionError extends MyDataAPIError {
	constructor(requestId?: string) {
...
export class NoAccessError extends MyDataAPIError {
	constructor(requestId?: string) {
...
export class ExpiredSessionKeyError extends MyDataAPIError {
	constructor(requestId?: string) {
...
export class InternalError extends MyDataAPIError {
	constructor(requestId?: string) {
		super('Internal server error, caller should retry', requestId);
	}
}

/** General server errors that are not specific to the MyData API (e.g., 404 "Not Found") */
export class GeneralError extends MyDataAPIError {}
