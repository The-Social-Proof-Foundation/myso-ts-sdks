...
import { bcs, fromBase64, fromHex, toBase64, toHex } from '@socialproof/bcs';
...
import type { KeyServerConfig, MyDataCompatibleClient } from './types.js';
...
/**
 * Given a list of key server object IDs, returns a list of MyDataKeyServer
 ...
 * @param client - The MySo client to use (MySoGrpcClient, MySoGraphQLClient, or MySoJsonRpcClient).
 * @param configs - The key server configurations containing aggregator URLs.
 * @returns - An array of MyDataKeyServer.
 */
...
export async function verifyKeyServer(
	...
	await MyDataAPIError.assertResponse(response, requestId);
 ...
 * Then decrypt the MyData key with ephemeral secret key. Returns a list decryption keys with
 * their full IDs.
 *
