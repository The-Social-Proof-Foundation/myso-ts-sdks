...
import { isValidNamedPackage, isValidMySoAddress, isValidMySoObjectId } from '@socialproof/myso/utils';
...
import type { MyDataCompatibleClient } from './types.js';
...
export class SessionKey {
	...
	private constructor({
		...
		if (!isValidMySoObjectId(packageId) || !isValidMySoAddress(address)) {
		...
		if (signer && signer.getPublicKey().toMySoAddress() !== address) {
	 ...
	 * @param mysoClient - The MySo client.
	...
	static async create({
		...
		const packageObj = await mysoClient.core.getObject({ objectId: packageId });
