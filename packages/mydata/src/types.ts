...
import type { ClientWithExtensions, CoreClient } from '@socialproof/myso/client';
import type { DemType, KemType } from './encrypt.js';
import type { SessionKey } from './session-key.js';

export type KeyCacheKey = `${string}:${string}`;
export type MyDataCompatibleClient = ClientWithExtensions<{
	core: CoreClient;
}>;

export interface MyDataOptions<Name = 'mydata'> {
	...
	timeout?: number;
	// Name of the mydata extension, defaults to 'mydata'
	name?: Name;
}
...
/** Configuration options for initializing a MyDataClient*/
export interface MyDataClientOptions {
...
export interface DecryptOptions {
	...
	/** The transaction bytes to use (that calls mydata_approve* functions). */
	txBytes: Uint8Array;
