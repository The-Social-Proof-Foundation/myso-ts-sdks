...
import { MyDataClient } from '../../src/index.js';
...
import { KeyCacheKey, MyDataCompatibleClient } from '../../src/types.js';
...
async function constructTxBytes(
	packageId: string,
	moduleName: string,
	mysoClient: MyDataCompatibleClient,
	innerIds: string[],
): Promise<Uint8Array> {
	...
	return await tx.build({ client: mysoClient, onlyTransactionKind: true });
}
...
describe('Integration test', () => {
	...
	let mysoClient: MyDataCompatibleClient;
	...
	beforeAll(async () => {
		keypair = Ed25519Keypair.fromSecretKey(
			'mysoprivkey1qqgzvw5zc2zmga0uyp4rzcgk42pzzw6387zqhahr82pp95yz0scscffh2d8',
		);
		mysoAddress = keypair.getPublicKey().toMySoAddress();
		mysoClient = new MySoGrpcClient({
			network: 'testnet',
	...
	async function runScenario(
		...
		const txBytes = await constructTxBytes(TESTNET_PACKAGE_ID, 'allowlist', mysoClient, [
			whitelistId,
		]);
		...
		// construct a ptb that contains two mydata_approve
		// for whitelist 1 and 2.
		const txBytes2 = await constructTxBytes(TESTNET_PACKAGE_ID, 'allowlist', mysoClient, [
			whitelistId,
			whitelistId2,
		]);
		...
		const client2 = new MyDataClient({
			mysoClient,
	...
	it('test decryption with LE nonce', { timeout: 12000 }, async () => {
		...
		// This was created with a version of the MyData SDK where the 'noble/curves' version was >=1.9.6 and thus the scalars were encoded in LE.
		...
		// This was created with a version of the MyData SDK where the 'noble/curves' version was <1.9.6 and thus the scalars were encoded in BE (as expected).
	...
	it('test getDerivedKeys with MVR name', { timeout: 12000 }, async () => {
		...
		const data = new Uint8Array([1, 2, 3]);
		const mvrName = '@pkg/mydata-demo-1234';
		...
		const txBytes = await constructTxBytes(packageId, 'allowlist', mysoClient, [whitelistId]);
	...
	it('client extension', { timeout: 12000 }, async () => {
		...
		// decrypt the object encrypted to whitelist 1.
		const decryptedBytes = await client.mydata.decrypt({
			data: encryptedBytes,
			sessionKey,
			txBytes,
		});
	...
	it('test different validateEncryptionServices errors', async () => {
		...
		const clientAllServers = new MyDataClient({
			mysoClient,
			serverConfigs: objectIds,
			verifyKeyServers: false,
		});
		...
		const client2Servers = new MyDataClient({
			mysoClient,
		...
		const client1Server = new MyDataClient({
			mysoClient,
		...
		const clientDifferentWeight = new MyDataClient({
			mysoClient,
			serverConfigs: objectIds,
			verifyKeyServers: false,
		});
	...
	it('test fetchKeys throws MyDataAPIError', async () => {
		...
		// Session key with mismatched myso address and personal msg signature fails.
		const wrongMySoAddress = Ed25519Keypair.generate().getPublicKey().toMySoAddress();
		const sessionKey = await SessionKey.create({
			address: wrongMySoAddress,
			packageId: TESTNET_PACKAGE_ID,
			ttlMin: 10,
			mysoClient,
		});
		...
		const wrongTxBytes = await constructTxBytes(TESTNET_PACKAGE_ID, 'allowlist', mysoClient, [
		...
		tx.transferObjects([objectArg], mysoAddress);
		const txBytes3 = await tx.build({ client: mysoClient, onlyTransactionKind: true });
	...
	it('test majority error with 3 out of 5 key servers', async () => {
		...
		vi.spyOn(mysoClient.core, 'getObject').mockResolvedValue({
			object: {
				version: '1',
			},
		} as any);
