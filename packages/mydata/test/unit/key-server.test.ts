...
// Data for mock response from MySoGrpcClient
const pk = fromBase64(
...
const keyType = 0;
const url = 'https://mydata-key-server-testnet-1.mystenlabs.com';
const name = 'mysten-testnet-v1-1';

// Helper to create mock MySoClient for V2 servers.
...
describe('key-server tests', () => {
	...
	it('test verifyKeyServer throws MyDataAPIError on 503', async () => {
		const keyServers = [
			{
				objectId: id,
				name,
	...
	it('test retrieveKeyServers throws error for committee server without aggregatorUrl', async () => {
		const mockMySoClient = createMockV2Client(id, 2, 2, 'Committee');
	...
	it('test retrieveKeyServers V2 uses server URL for independent servers', async () => {
		const mockMySoClient = createMockV2Client(id, 2, 2, 'Independent', url);
	...
	it('test version selection prefers highest supported version', async () => {
		const mockMySoClient = createMockV2Client(id, 1, 2, 'Independent'); // Supports both V1 and V2

		await retrieveKeyServers({ objectIds: [id], client: mockMySoClient, configs: new Map() });

		// Verify getDynamicField was called with version 2 (highest supported)
		expect(mockMySoClient.core.getDynamicField).toHaveBeenCalledWith(
