...
- e00788c: Consolidate GraphQL schema exports into a single version:
  - Removed versioned schema exports: `@socialproof/myso/graphql/schemas/2024.1`,
    `@socialproof/myso/graphql/schemas/2024.4`, `@socialproof/myso/graphql/schemas/latest`
  - Added single unified schema export: `@socialproof/myso/graphql/schema`
...
- e00788c: `network` is now a required option on MySoGraphQLClient and MySoJsonRpcClient
...
- e00788c: Remove MySoClient exports from @socialproof/myso/client

  BREAKING CHANGE: All exports from `@socialproof/myso/client` have been removed. Use
  `@socialproof/myso/jsonRpc` instead:
  - `MySoClient` -> `MySoJsonRpcClient`
  - `MySoClientOptions` -> `MySoJsonRpcClientOptions`
  - `isMySoClient` -> `isMySoJsonRpcClient`
  - `MySoTransport` -> `JsonRpcTransport`
  ...
  const client = new MySoClient({ url: getFullnodeUrl('devnet'), network: 'devnet' });

  // After
  import { MySoJsonRpcClient, getJsonRpcFullnodeUrl } from '@socialproof/myso/jsonRpc';
  const client = new MySoJsonRpcClient({ url: getJsonRpcFullnodeUrl('devnet'), network: 'devnet' });
  ```
...
- e00788c: Remove deprecated exports and properties:
  ...
  - Removed deprecated faucet methods: `requestMySoFromFaucetV0`, `requestMySoFromFaucetV1`,
    `getFaucetRequestStatus` (use `requestMySoFromFaucetV2` instead)
...
- e00788c: Stabilize experimental client API by moving it from `@socialproof/myso/experimental` to
  ...
  Breaking changes:
  - `@socialproof/myso/experimental` module has been removed
...
- e00788c: Update transaction executor APIs to accept `ClientWithCoreApi` for gRPC, GraphQL, and
  ...
  **Breaking Changes:**
  - `CachingTransactionExecutor`, `SerialTransactionExecutor`, and `ParallelTransactionExecutor` now
    accept any client implementing `ClientWithCoreApi` instead of requiring `MySoJsonRpcClient`
  - The `executeTransaction()` return type changed: `data` property renamed to `result` and uses
    core API types (`MySoClientTypes.TransactionResult`)
...
- d554cd2: Add new @socialproof/myso/jsonRpc export
...
- ec519fc: Deprecate schema on `ParsedKeypair` returned by `decodeMySoPrivateKey` which should have
...
- 19a8045: Add verifyZkLoginSignature to core API and support MySoClient in verifySignature methods
...
- 4d13ef8: Add abort signals to all MySoClient methods
...
- 539168a: expose isArgument util from @socialproof/myso/transactions
...
- ec2dc7f: All functionality from `@socialproof/zklogin` has been moved to
  ...
  + import { computeZkLoginAddress, jwtToAddress } from '@socialproof/myso/zklogin';
...
- 929db4976a: Add normalizeMySoNSName and isValidMySoNSName utils, and add a format option to
  MySoClient.resolveNameServiceNames
...
- 220a766d86: Fix WebSocket constructor not being properly assigned in MySoClient HTTP transport
...
- cdcfa76c43: Add a new client method for retrieving epoch metrics (mysox_getEpochMetrics)
...
- 0259aec82: Removed dependency on @open-rpc/client-js and replaced it with standard fetch and
  ...
  need to provide a WebSocket implementation when creating your MySoClient. You can either use a
  global polyfill for the WebSocket class, or pass a compatible WebSocket implementation into
  MySoHTTPTransport (eg, using the `ws` package)

  ```typescript
  import { getFullnodeUrl, MySoClient, MySoHTTPTransport } from '@socialproof/myso.js/client';
...
- fd8589806: Remove all previously deprecated exports from @socialproof/myso.js
...
- a503cad34: Add exports to `@socialproof/myso.js/client` for rpc method params
...
- cce6ffbcc: Add toMySoPublicKey method for retrieving the MySo representation of a raw public key
...
- cc6441f46: The MySo TS SDK has been broken up into a set of modular exports, and all exports from
  the root of the package have been deprecated. The following export paths have been added:
  - `@socialproof/myso.js/client` - A client for interacting with MySo RPC nodes.
  - `@socialproof/myso.js/bcs` - A BCS builder with pre-defined types for MySo.
  - `@socialproof/myso.js/transaction` - Utilities for building and interacting with transactions.
  - `@socialproof/myso.js/keypairs/*` - Modular exports for specific KeyPair implementations.
  - `@socialproof/myso.js/verify` - Methods for verifying transactions and messages.
  - `@socialproof/myso.js/cryptography` - Shared types and classes for cryptography.
  - `@socialproof/myso.js/multisig` - Utilities for working with multisig signatures.
  - `@socialproof/myso.js/utils` - Utilities for formatting and parsing various MySo types.
  - `@socialproof/myso.js/faucet`- Methods for requesting myso from a faucet.

  As part of this refactor we are deprecating a number of existing APIs:
  - `JsonRPCProvider` - This Provider pattern is being replaced by a new `MySoClient`
  - `SignerWithProver` and `RawSigner` - The Concept of Signers is being removed from the SDK.
    ...
    `signAndExecuteTransactionBlock` method has been moved to the new `MySoClient`.
  - The `superstruct` type definitions for types used by JsonRPCProvider are being replaced with
    generated types exported from `@socialproof/myso.js/client`. The new type definitions are pure
  ...
  The new MySoClient should mostly work as a drop in replacement for the `JsonRpcProvider` provider.
  Setting up a `MySoClient` is slightly different, but once constructed should work just like a
  provider.

  ```diff
  - import { JsonRpcProvider, devnetConnection } from '@socialproof/myso.js';
  + import { MySoClient, getFullnodeUrl } from '@socialproof/myso.js/client';

  - const provider = new JsonRpcProvider(localnetConnection);
  + const client = new MySoClient({ url: getFullnodeUrl('localnet')});
  ```
  ...
  -    TransactionBlock,
  -    localnetConnection,
  - } from '@socialproof/myso.js';
  + import { Ed25519Keypair } from '@socialproof/myso.js/keypairs/ed25519';
  + import { MySoClient, getFullnodeUrl } from '@socialproof/myso.js/client';
  + import { TransactionBlock } from '@socialproof/myso.js/transactions';
  ...
  The ability to request MySo from a faucet was not added to `MySoClient`, instead you will need to
  use a method `@socialproof/myso.js/faucet` to make these requests

  ```diff
  - import { JsonRpcProvider, devnetConnection } from '@socialproof/myso.js';
  - const provider = new JsonRpcProvider(devnetConnection);
  + import { requestMySoFromFaucetV0, getFaucetHost } from '@socialproof/myso.js/faucet';

  - await provider.requestMySoFromFaucet(
  -  '<YOUR MYSO ADDRESS>'
  - );
  + await requestMySoFromFaucetV0({
  +   host: getFaucetHost('devnet'),
  +   recipient: '<YOUR MYSO ADDRESS>',
  +});
  ```

- 001148443: Introduce new `@socialproof/myso.js/faucet` export, which should be used for all faucet
  interactions. This deprecates the previous `requestMySoFromFaucet` APIs that existed on the
...
- 2f37537d5: Update `MySoEventFilter` structure for `TimeRange` query.
...
- 09d77325a9: Add new MySoNS Toolkit package.
...
- acc2edb31: Update schema for `MySoSystemState` and `DelegatedStake`
- 941b03af1: Change functions in transactions.ts of ts-sdk such that: `getTotalGasUsed` and
  ...
  `string`, `epochId` is defined as `string`. In `myso-json-rpc` the corresponding types are defined
  as `BigInt`. Introduce `MySoEpochId` type to `myso-json-rpc` types that is a `BigInt`.
...
- 33cb357e1: Change functions in json-rpc-provider.ts of ts-sdk such that:
  ...
  `cursor` of `getCheckpoints` is defined as a `string`. Introduce `MySoCheckpointSequenceNumber`
  type in myso-json-rpc-types that is a `BigInt` to use instead of `CheckpointSequenceNumber` of
  myso-types.
...
- f1e42f792: Consolidate get_object and get_raw_object into a single get_object endpoint which now
  takes an additional config parameter with type `MySoObjectDataOptions` and has a new return type
  `MySoObjectResponse`. By default, only object_id, version, and digest are fetched.
- 272389c20: Support for new versioned TransactionData format
- 3de8de361: Remove `getMySoSystemState` method. Use `getLatestMySoSystemState` method instead.
...
- 7a2eaf4a3: Changing the MySoObjectResponse struct to use data/error fields instead of
  details/status
...
- 0a7b42a6d: This changes almost all occurences of "delegate", "delegation" (and various
  ...
  convention.
- 3de8de361: Remove `getValidators` API. Use `getLatestMySoSystemState` instead.
...
- 65f1372dd: Rename `provider.getTransactionWithEffects` to `provider.getTransaction`. The new
  method takes in an additional parameter `MySoTransactionBlockResponseOptions` to configure which
...
- 3304eb83b: Refactor Rust MySoTransactionBlockKind to be internally tagged for Json serialization
  with tag="type" and MySoEvent to be adjacently tagged with tag="type" and content="content"
...
- 64fb649eb: Remove old `MySoExecuteTransactionResponse` interface, and `CertifiedTransaction`
  interface in favor of the new unified `MySoTransactionBlockResponse` interfaces.
...
- ebe6c3945: Support deserializing `payMySo` and `payAllMySo` transactions
...
- 5cd51dd38: Deprecate myso_executeTransaction in favor of myso_executeTransactionSerializedSig
...
- d2015f815: Rebuilt type-narrowing utilties (e.g. `isMySoObject`) on top of Superstruct, which
  ...
  used to replace the previous narrowing functions. For example, `isMySoObject(data)` becomes
  `is(data, MySoObject)`.
...
- ea71d8216: Use intent signing if myso version > 0.18
...
- db22728c1: \* adds dryRunTransaction support
  - adds getGasCostEstimation to the signer-with-provider that estimates the gas cost for a
    transaction
- 3b510d0fc: adds coin transfer method to framework that uses pay and payMySo
...
- e45b188a8: Introduce PayMySo and PayAllMySo native transaction types to TS SDK.
...
- 4575c0a02: Fix type definition of MySoMoveNormalizedType
