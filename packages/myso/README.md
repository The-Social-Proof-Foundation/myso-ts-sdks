...
For more complete docs, visit the [MySo TypeScript SDK docs](https://sdk.mystenlabs.com/)

# MySo TypeScript SDK

This is the MySo TypeScript SDK providing utility classes and functions for applications to sign
transactions and interact with the MySo network. It supports multiple communication protocols
...
# Run `build` for the TypeScript SDK if you're in the `sdk/myso` project
$ pnpm run build

# Run `sdk build` for the TypeScript SDK if you're in the root of `myso` repo
$ pnpm sdk build
```

> All `pnpm` commands below are intended to be run in the root of the MySo repo.

## Type Doc

You can view the generated [Type Doc](https://typedoc.org/) for the
[current release of the SDK](https://www.npmjs.com/package/@socialproof/myso) at
...
```
pnpm --filter @socialproof/myso test:unit
```

To run E2E tests against local network

```
pnpm --filter @socialproof/myso prepare:e2e

// This will run all e2e tests
pnpm --filter @socialproof/myso test:e2e
...
```
VITE_FAUCET_URL='https://faucet.devnet.mysocial.network:443/v2/gas' VITE_FULLNODE_URL='https://fullnode.devnet.mysocial.network' pnpm --filter @socialproof/myso exec vitest e2e
```

## Connecting to MySo Network

The `MySoGrpcClient` class provides a connection to the MySo network via gRPC and is the recommended
...
[this guide](https://docs.mysocial.network/build/myso-local-network) for more information.
...
You can also construct your own MySoGrpcClient connections with the URL for your own fullnode:
...
You can request myso from the faucet when running against devnet or localnet. For testnet, visit
...
tx.transferObjects([coin], keypair.getPublicKey().toMySoAddress());
const result = await client.signAndExecuteTransaction({
	signer: keypair,
	transaction: tx,
});
...
tx.transferObjects([upgradeCap], keypair.toMySoAddress());
const result = await client.signAndExecuteTransaction({
	signer: keypair,
	transaction: tx,
});
...
// If coin type is not specified, it defaults to 0x2::myso::MYSO
