...
import { isValidMySoAddress, normalizeMySoAddress, MYSO_ADDRESS_LENGTH } from '../utils/myso-types.js';
...
export const Address = bcs.bytes(MYSO_ADDRESS_LENGTH).transform({
	validate: (val) => {
		const address = typeof val === 'string' ? val : toHex(val);
		if (!address || !isValidMySoAddress(normalizeMySoAddress(address))) {
			throw new Error(`Invalid MySo address ${address}`);
		}
	},
	input: (val: string | Uint8Array) =>
		typeof val === 'string' ? fromHex(normalizeMySoAddress(val)) : val,
	output: (val) => normalizeMySoAddress(toHex(val)),
});
...
export const MySoObjectRef = bcs.struct('MySoObjectRef', {
	objectId: Address,
	version: bcs.u64(),
	digest: ObjectDigest,
});
...
export const ObjectArg = bcs.enum('ObjectArg', {
	ImmOrOwnedObject: MySoObjectRef,
	SharedObject: SharedObjectRef,
	Receiving: MySoObjectRef,
});

// Rust: crates/myso-types/src/object.rs
...
export const GasData = bcs.struct('GasData', {
	payment: bcs.vector(MySoObjectRef),
	owner: Address,
	price: bcs.u64(),
	budget: bcs.u64(),
});
