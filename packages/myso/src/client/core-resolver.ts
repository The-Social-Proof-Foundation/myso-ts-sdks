...
import { normalizeMySoAddress, normalizeMySoObjectId, MYSO_TYPE_ARG } from '../utils/index.js';
...
function getClient(options: BuildTransactionOptions): ClientWithCoreApi {
	if (!options.client) {
		throw new Error(
			`No myso client passed to Transaction#build, but transaction data was not sufficient to build offline.`,
		);
	}
...
async function setGasPayment(transactionData: TransactionDataBuilder, client: ClientWithCoreApi) {
	if (!transactionData.gasData.payment) {
		...
		const [mysoBalance, coins] = await Promise.all([
		...
		if (
			mysoBalance?.balance.addressBalance &&
			BigInt(mysoBalance.balance.addressBalance) >=
...
async function resolveObjectReferences(
	...
	const dedupedIds = [
		...new Set(
			objectsToResolve.map((input) => normalizeMySoObjectId(input.UnresolvedObject.objectId)),
		),
	];
...
async function normalizeInputs(transactionData: TransactionDataBuilder, client: ClientWithCoreApi) {
	...
	const moveFunctionParameters = new Map<string, MySoClientTypes.OpenSignature[]>();
...
function isReceivingType(type: MySoClientTypes.OpenSignature): boolean {
