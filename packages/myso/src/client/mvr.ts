...
import { chunk, DataLoader } from '@socialproof/utils';
import { isValidNamedPackage, isValidNamedType } from '../utils/move-registry.js';
import type { StructTag } from '../utils/myso-types.js';
import {
	isValidMySoAddress,
	normalizeStructTag,
...
const MVR_API_HEADER = {
	'Mvr-Source': `@socialproof/myso@${PACKAGE_VERSION}`,
};
...
export class MvrClient implements MySoClientTypes.MvrMethods {
	#cache: ClientCache;
	#url?: string;
	#pageSize: number;
	#overrides: {
	...
	async resolvePackage({
		package: name,
	}: MySoClientTypes.MvrResolvePackageOptions): Promise<MySoClientTypes.MvrResolvePackageResponse> {
		if (!hasMvrName(name)) {
			return {
				package: name,
			};
	...
	async resolveType({
		type,
	}: MySoClientTypes.MvrResolveTypeOptions): Promise<MySoClientTypes.MvrResolveTypeResponse> {
		if (!hasMvrName(type)) {
			return {
				type,
			};
	...
	async resolve({
		types = [],
		packages = [],
	}: MySoClientTypes.MvrResolveOptions): Promise<MySoClientTypes.MvrResolveResponse> {
...
function validateOverrides(overrides?: {
	...
	if (overrides?.packages) {
		for (const [pkg, id] of Object.entries(overrides.packages)) {
			...
			if (!isValidMySoAddress(normalizeMySoAddress(id))) {
	...
	if (overrides?.types) {
		for (const [type, val] of Object.entries(overrides.types)) {
			...
			if (!isValidMySoAddress(parsedValue.address)) {
...
export function hasMvrName(nameOrType: string) {
	return (
		nameOrType.includes(NAME_SEPARATOR) || nameOrType.includes('@') || nameOrType.includes('.myso')
	);
}
...
export function replaceNames(
	...
	resolved: MySoClientTypes.MvrResolveResponse,
) {
	for (const command of builder.commands) {
