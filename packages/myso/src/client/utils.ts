...
function parseBcsExecutionError(failure: {
	...
	switch (error.$kind) {
		...
		case 'MySoMoveVerificationError':
		...
		case 'MySoMoveVerificationTimedout':
...
): MySoClientTypes.TransactionData {
	return (
		onlyTransactionKind
...
export function parseTransactionEffectsBcs(effects: Uint8Array): MySoClientTypes.TransactionEffects {
...
}): MySoClientTypes.TransactionEffects {
...
function parseTransactionEffectsV2({
	...
	const changedObjects = effects.changedObjects.map(
		([id, change]): MySoClientTypes.ChangedObject => {
			return {
				objectId: id,
	...
	return {
		...
		unchangedConsensusObjects: effects.unchangedConsensusObjects.map(
			([objectId, object]): MySoClientTypes.UnchangedConsensusObject => {
				return {
					kind: object.$kind,
					objectId: objectId,
					version:
