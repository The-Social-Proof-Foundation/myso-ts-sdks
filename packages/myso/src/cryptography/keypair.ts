...
import type { ClientWithCoreApi, MySoClientTypes } from '../client/index.js';

export const PRIVATE_KEY_SIZE = 32;
export const LEGACY_PRIVATE_KEY_SIZE = 64;
export const MYSO_PRIVATE_KEY_PREFIX = 'mysoprivkey';
...
export abstract class Signer {
	...
	async signAndExecuteTransaction({
		...
		MySoClientTypes.TransactionResult<{ transaction: true; effects: true }>
	> {
		transaction.setSenderIfNotSet(this.toMySoAddress());
	...
	toMySoAddress(): string {
		return this.getPublicKey().toMySoAddress();
	}
 ...
 * 33-byte Bech32 encoded string starting with `mysoprivkey`, and
 * parse out the signature scheme and the private key in bytes.
 */
export function decodeMySoPrivateKey(value: string): ParsedKeypair {
	const { prefix, words } = bech32.decode(value as `${string}1${string}`);
	if (prefix !== MYSO_PRIVATE_KEY_PREFIX) {
...
/**
 * This returns a Bech32 encoded string starting with `mysoprivkey`,
 * encoding 33-byte `flag || bytes` for the given the 32-byte private
 * key and its signature scheme.
 */
export function encodeMySoPrivateKey(bytes: Uint8Array, scheme: SignatureScheme): string {
	...
	return bech32.encode(MYSO_PRIVATE_KEY_PREFIX, bech32.toWords(privKeyBytes));
}
