...
import type { GraphQLQueryOptions, MySoGraphQLClient } from './client.js';
import type {
...
import { BalanceChange as BalanceChangeType } from '../grpc/proto/myso/rpc/v2/balance_change.js';
import { TransactionEffects as TransactionEffectsType } from '../grpc/proto/myso/rpc/v2/effects.js';
import { Transaction as GrpcTransactionType } from '../grpc/proto/myso/rpc/v2/transaction.js';
...
export class GraphQLCoreClient extends CoreClient {
	...
	async listOwnedObjects<Include extends MySoClientTypes.ObjectInclude = object>(
		...
		return {
			objects: objects.nodes.map(
				(obj): MySoClientTypes.Object<Include> => ({
					objectId: obj.address,
					version: obj.version?.toString()!,
					digest: obj.digest!,
					owner: mapOwner(obj.owner!),
					...
					objectBcs: (obj.objectBcs
						...
						: undefined) as MySoClientTypes.Object<Include>['objectBcs'],
	...
	async getMoveFunction(
		...
		return {
			function: {
				packageId: normalizeMySoAddress(options.packageId),
				...
				parameters:
					moveFunction.parameters?.map((param) => parseNormalizedMySoMoveType(param.signature)) ??
					[],
				returns:
					moveFunction.return?.map(({ signature }) => parseNormalizedMySoMoveType(signature)) ?? [],
			},
		};
	}
...
function mapOwner(owner: Object_Owner_FieldsFragment): MySoClientTypes.ObjectOwner {
...
function parseTransaction<Include extends MySoClientTypes.TransactionInclude = object>(
	...
	let balanceChanges: MySoClientTypes.BalanceChange[] | undefined;
	...
	const status: MySoClientTypes.ExecutionStatus =
	...
	const result: MySoClientTypes.Transaction<Include> = {
		...
		balanceChanges: balanceChanges as MySoClientTypes.Transaction<Include>['balanceChanges'],
...
function parseNormalizedMySoMoveType(type: OpenMoveTypeSignature): MySoClientTypes.OpenSignature {
...
): MySoClientTypes.ExecutionError {
...
): MySoClientTypes.MoveLocation | undefined {
...
function parseGraphQLCleverError(
	executionError: GraphQLExecutionError,
): MySoClientTypes.CleverError | undefined {
...
function parseNormalizedMySoMoveTypeBody(
	...
	if ('datatype' in type) {
		return {
			$kind: 'datatype',
			datatype: {
				typeName: `${normalizeMySoAddress(type.datatype.package)}::${type.datatype.module}::${type.datatype.type}`,
				typeParameters: type.datatype.typeParameters.map((t) => parseNormalizedMySoMoveTypeBody(t)),
			},
		};
	}
