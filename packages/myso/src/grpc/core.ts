...
import type { CoreClientOptions, MySoClientTypes } from '../client/index.js';
import { CoreClient, formatMoveAbortMessage, SimulationError } from '../client/index.js';
import type { MySoGrpcClient } from './client.js';
import type { Owner } from './proto/myso/rpc/v2/owner.js';
import { Owner_OwnerKind } from './proto/myso/rpc/v2/owner.js';
import { DynamicField_DynamicFieldKind } from './proto/myso/rpc/v2/state_service.js';
import { chunk, fromBase64, toBase64 } from '@socialproof/utils';
import type { ExecutedTransaction } from './proto/myso/rpc/v2/executed_transaction.js';
import type { TransactionEffects } from './proto/myso/rpc/v2/effects.js';
import {
...
} from './proto/myso/rpc/v2/effects.js';
import type {
	ExecutionError as GrpcExecutionError,
	MoveAbort as GrpcMoveAbort,
} from './proto/myso/rpc/v2/execution_status.js';
import {
...
import { normalizeStructTag, normalizeMySoAddress } from '../utils/myso-types.js';
import { MYSO_TYPE_ARG } from '../utils/constants.js';
import type { OpenSignature, OpenSignatureBody } from './proto/myso/rpc/v2/move_package.js';
import {
	Ability,
	FunctionDescriptor_Visibility,
	OpenSignature_Reference,
	OpenSignatureBody_Type,
} from './proto/myso/rpc/v2/move_package.js';
import {
...
export class GrpcCoreClient extends CoreClient {
	...
	async getObjects<Include extends MySoClientTypes.ObjectInclude = object>(
		...
		for (const batch of batches) {
			...
			results.push(
				...response.response.objects.map((object): MySoClientTypes.Object<Include> | Error => {
					...
					return {
						...
						content: bcsContent as MySoClientTypes.Object<Include>['content'],
	...
	async listOwnedObjects<Include extends MySoClientTypes.ObjectInclude = object>(
		...
		const objects = response.response.objects.map(
			(object): MySoClientTypes.Object<Include> => ({
				objectId: object.objectId!,
				version: object.version?.toString()!,
				digest: object.digest!,
				content: object.contents?.value as MySoClientTypes.Object<Include>['content'],
				owner: mapOwner(object.owner)!,
				type: object.objectType!,
				previousTransaction: (object.previousTransaction ??
					undefined) as MySoClientTypes.Object<Include>['previousTransaction'],
				objectBcs: object.bcs?.value as MySoClientTypes.Object<Include>['objectBcs'],
				json: (options.include?.json
					? object.json
						? (Value.toJson(object.json) as Record<string, unknown>)
						: null
					: undefined) as MySoClientTypes.Object<Include>['json'],
			}),
		);
	...
	async listCoins(
		...
		const coinType = options.coinType ?? MYSO_TYPE_ARG;
	...
	async getReferenceGasPrice(): Promise<MySoClientTypes.GetReferenceGasPriceResponse> {
	...
	async getCurrentSystemState(): Promise<MySoClientTypes.GetCurrentSystemStateResponse> {
		const response = await this.#client.ledgerService.getEpoch({
			readMask: {
				paths: [
	...
	async getMoveFunction(
		...
		return {
			function: {
				...
				parameters:
					response.function?.parameters?.map((param) => parseNormalizedMySoMoveType(param)) ?? [],
				returns: response.function?.returns?.map((ret) => parseNormalizedMySoMoveType(ret)) ?? [],
			},
		};
	}
...
function mapOwner(owner: Owner | null | undefined): MySoClientTypes.ObjectOwner | null {
	if (!owner) {
		return null;
	}
...
function parseGrpcExecutionError(error: GrpcExecutionError): MySoClientTypes.ExecutionError {
...
function parseMoveAbort(abort: GrpcMoveAbort): MySoClientTypes.MoveAbort {
	return {
		abortCode: String(abort.abortCode ?? 0n),
		location: { ...abort.location },
...
): null | MySoClientTypes.UnchangedConsensusObject['kind'] {
	if (kind == null) {
		return null;
	}
	switch (kind) {
...
export function parseTransactionEffects({
...
}): MySoClientTypes.TransactionEffects | null {
	if (!effects) {
		return null;
	}

	const changedObjects = effects.changedObjects.map((change): MySoClientTypes.ChangedObject => {
		return {
	...
	return {
		...
		unchangedConsensusObjects: effects.unchangedConsensusObjects.map(
			(object): MySoClientTypes.UnchangedConsensusObject => {
				return {
...
function parseTransaction<Include extends MySoClientTypes.TransactionInclude = object>(
	...
	const status: MySoClientTypes.ExecutionStatus = transaction.effects?.status?.success
	...
	const result: MySoClientTypes.Transaction<Include> = {
		...
		status,
		effects: effects as MySoClientTypes.Transaction<Include>['effects'],
		...
		events: (include?.events
			? (transaction.events?.events.map((event) => ({
					packageId: normalizeMySoAddress(event.packageId!),
					module: event.module!,
					sender: normalizeMySoAddress(event.sender!),
...
function parseNormalizedMySoMoveType(type: OpenSignature): MySoClientTypes.OpenSignature {
	...
	return {
		reference,
		body: parseNormalizedMySoMoveTypeBody(type.body!),
	};
}

function parseNormalizedMySoMoveTypeBody(type: OpenSignatureBody): MySoClientTypes.OpenSignatureBody {
	switch (type.type) {
		...
		case OpenSignatureBody_Type.VECTOR:
			return {
				$kind: 'vector',
				vector: parseNormalizedMySoMoveTypeBody(type.typeParameterInstantiation[0]),
			};
			...
			return {
				...
				datatype: {
					...
					typeParameters: type.typeParameterInstantiation.map((t) =>
						parseNormalizedMySoMoveTypeBody(t),
					),
				},
			};
