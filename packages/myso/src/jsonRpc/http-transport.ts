...
import { JsonRpcError, MySoHTTPStatusError } from './errors.js';
...
export class JsonRpcHTTPTransport implements JsonRpcTransport {
	...
	fetch(input: RequestInfo, init?: RequestInit): Promise<Response> {
		...
		if (!fetchFn) {
			throw new Error(
				'The current environment does not support fetch, you can provide a fetch implementation in the options for MySoHTTPTransport.',
			);
		}
	...
	#getWebsocketClient(): WebsocketClient {
		if (!this.#websocketClient) {
			...
			if (!WebSocketConstructor) {
				throw new Error(
					'The current environment does not support WebSocket, you can provide a WebSocketConstructor in the options for MySoHTTPTransport.',
				);
			}
