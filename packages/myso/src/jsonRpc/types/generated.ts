 ...
 * /crates/myso-open-rpc/spec/openrpc.json
 */
...
export interface DevInspectResults {
	...
	results?: MySoExecutionResult[] | null;
}
...
export type MySoMoveFunctionArgType =
	| 'Pure'
	| {
...
export interface MySoObjectData {
	/**
	 * Move object content or package content in BCS, default to be None unless
	 * MySoObjectDataOptions.showBcs is set to true
	 */
	bcs?: RawData | null;
	/**
	 * Move object content or package content, default to be None unless MySoObjectDataOptions.showContent
	 * is set to true
	 */
	content?: MySoParsedData | null;
	/** Base64 string representing the object digest */
	digest: string;
	/**
	 * The Display metadata for frontend UI rendering, default to be None unless
	 * MySoObjectDataOptions.showContent is set to true This can also be None if the struct type does not
	...
	/**
	 * The amount of MYSO we would rebate if this object gets deleted. This number is re-calculated each
...
export interface MySoObjectDataOptions {
...
export interface MySoObjectRef {
	/** Base64 string representing the object digest */
	digest: string;
...
export interface MySoObjectResponseQuery {
	/** If None, no filter will be applied */
	filter?: MySoObjectDataFilter | null;
	/** config which fields to include in the response, by default only digest is included */
	options?: MySoObjectDataOptions | null;
}
export type ObjectValueKind = 'ByImmutableReference' | 'ByMutableReference' | 'ByValue';
export interface OwnedObjectRef {
	owner: ObjectOwner;
	reference: MySoObjectRef;
}
export type ObjectOwner =
	 ...
	 * MySoAddress as MySoAddress is universal.
	 */
	| {
			ObjectOwner: string;
...
export interface PaginatedTransactionResponse {
	data: MySoTransactionBlockResponse[];
	hasNextPage: boolean;
	nextCursor?: string | null;
}
/**
...
export type Signature =
	| {
			Ed25519MySoSignature: string;
	  }
	| {
			Secp256k1MySoSignature: string;
	  }
	| {
			Secp256r1MySoSignature: string;
	  };
...
export interface MySoActiveJwk {
	epoch: string;
	jwk: MySoJWK;
	jwk_id: MySoJwkId;
}
/** An argument to a transaction in a programmable transaction block */
export type MySoArgument =
...
export interface MySoAuthenticatorStateExpire {
	min_epoch: string;
}
export type MySoCallArg =
	| {
			type: 'object';
			digest: string;
			objectId: string;
	...
	| {
			reservation: MySoReservation;
			type: 'fundsWithdrawal';
			typeArg: MySoWithdrawalTypeArg;
			withdrawFrom: MySoWithdrawFrom;
	  };
export interface MySoChangeEpoch {
...
export type MySoEndOfEpochTransactionKind =
...
export interface MySoExecutionResult {
	/** The value of any arguments that were mutably borrowed. Non-mut borrowed values are not included */
	mutableReferenceOutputs?: [MySoArgument, number[], string][];
	/** The return values from the transaction */
	returnValues?: [number[], string][];
}
export interface MySoJWK {
	alg: string;
	e: string;
	kty: string;
	n: string;
}
export interface MySoJwkId {
	iss: string;
	kid: string;
}
export type MySoMoveAbility = 'Copy' | 'Drop' | 'Store' | 'Key';
export interface MySoMoveAbilitySet {
	abilities: MySoMoveAbility[];
}
export interface MySoMoveAbort {
	error_code?: string | null;
	function?: string | null;
	line?: number | null;
	module_id?: string | null;
}
export interface MySoMoveModuleId {
	address: string;
	name: string;
}
export interface MySoMoveNormalizedEnum {
	abilities: MySoMoveAbilitySet;
	typeParameters: MySoMoveStructTypeParameter[];
	variantDeclarationOrder?: string[] | null;
	variants: {
		[key: string]: MySoMoveNormalizedField[];
	};
}
export interface MySoMoveNormalizedField {
	name: string;
	type: MySoMoveNormalizedType;
}
export interface MySoMoveNormalizedFunction {
	isEntry: boolean;
	parameters: MySoMoveNormalizedType[];
	return: MySoMoveNormalizedType[];
	typeParameters: MySoMoveAbilitySet[];
	visibility: MySoMoveVisibility;
}
export interface MySoMoveNormalizedModule {
	address: string;
	enums?: {
		[key: string]: MySoMoveNormalizedEnum;
	};
	exposedFunctions: {
		[key: string]: MySoMoveNormalizedFunction;
	};
	fileFormatVersion: number;
	friends: MySoMoveModuleId[];
	name: string;
	structs: {
		[key: string]: MySoMoveNormalizedStruct;
	};
}
export interface MySoMoveNormalizedStruct {
	abilities: MySoMoveAbilitySet;
	fields: MySoMoveNormalizedField[];
	typeParameters: MySoMoveStructTypeParameter[];
}
export type MySoMoveNormalizedType =
	| 'Bool'
	| 'U8'
	| 'U16'
	| 'U32'
	...
	| {
			Vector: MySoMoveNormalizedType;
	  }
	| {
			TypeParameter: number;
	  }
	| {
			Reference: MySoMoveNormalizedType;
	  }
	| {
			MutableReference: MySoMoveNormalizedType;
	  };
export interface MySoMoveStructTypeParameter {
	constraints: MySoMoveAbilitySet;
	isPhantom: boolean;
}
export type MySoMoveVisibility = 'Private' | 'Public' | 'Friend';
export type MySoObjectDataFilter =
	| {
			MatchAll: MySoObjectDataFilter[];
	  }
	| {
			MatchAny: MySoObjectDataFilter[];
	  }
	| {
			MatchNone: MySoObjectDataFilter[];
...
export interface MySoObjectResponse {
	data?: MySoObjectData | null;
	error?: ObjectResponseError | null;
}
/**
 ...
 */
export interface MoveCallMySoTransaction {
	/** The arguments to the function. */
	arguments?: MySoArgument[];
...
export type MySoReservation = {
	maxAmountU64: string;
};
/**
 * This is the JSON-RPC type for the MYSO system state object. It flattens all fields to make them
 * top-level fields such that it as minimum dependencies to the internal data structures of the MYSO
 * system state type.
 */
export interface MySoSystemStateSummary {
	/** The list of active validators in the current epoch. */
	activeValidators: MySoValidatorSummary[];
...
export type MySoTransaction =
	/** A call to either an entry or a public Move function */
	| {
			MoveCall: MoveCallMySoTransaction;
	  } /**
	...
	| {
			Upgrade: [string[], string, MySoArgument];
	  } /**
	 ...
	 */
	| {
			MakeMoveVec: [string | null, MySoArgument[]];
	  };
export type MySoTransactionBlockBuilderMode = 'Commit' | 'DevInspect';
/**
 * This is the JSON-RPC type for the MYSO validator. It flattens all inner structures to top-level
 * fields so that they are decoupled from the internal definitions.
 */
export interface MySoValidatorSummary {
	...
	pendingTotalMySoWithdraw: string;
	...
	stakingPoolMySoBalance: string;
	mysoAddress: string;
	votingPower: string;
	workerAddress: string;
	workerPubkeyBytes: string;
}
export type MySoWithdrawFrom = 'sender' | 'sponsor';
export type MySoWithdrawalTypeArg = {
	balance: string;
};
export interface CoinSupply {
	value: string;
}
export interface MySoTransactionBlock {
...
export type TransactionEffects =
	/** The response from processing a transaction or a certified transaction */
	{
		...
		unwrappedThenDeleted?: MySoObjectRef[];
		/** Object refs of objects now wrapped in other objects. */
		wrapped?: MySoObjectRef[];
	};
export interface TransactionBlockEffectsModifiedAtVersions {
	objectId: string;
	sequenceNumber: string;
}
export type MySoTransactionBlockKind =
	...
	| {
			...
			transactions: MySoEndOfEpochTransactionKind[];
	  }
	| {
			commit_timestamp_ms: string;
...
export interface MySoTransactionBlockResponse {
	...
	transaction?: MySoTransactionBlock | null;
}
export interface MySoTransactionBlockResponseOptions {
...
export interface MySoTransactionBlockResponseQuery {
	/** If None, no filter will be applied */
	filter?: TransactionFilter | null;
	/** config which fields to include in the response, by default only digest is included */
	options?: MySoTransactionBlockResponseOptions | null;
}
