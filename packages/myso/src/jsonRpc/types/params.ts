...
export interface ExecuteTransactionBlockParams {
	...
	options?: RpcTypes.MySoTransactionBlockResponseOptions | null | undefined;
...
export interface GetObjectParams {
	...
	id: string;
	/** options for specifying the content to be returned */
	options?: RpcTypes.MySoObjectDataOptions | null | undefined;
	signal?: AbortSignal;
}
/**
...
export interface GetBalanceParams {
	 ...
	 * default to 0x2::myso::MYSO if not specified.
	 */
...
export interface GetLatestMySoSystemStateParams {
	signal?: AbortSignal;
}
/**
 ...
 * been updated when the next page is fetched. Please use mysox_queryObjects if this is a concern.
 */
export type GetOwnedObjectsParams = {
	...
	limit?: number | null | undefined;
	signal?: AbortSignal;
} & RpcTypes.MySoObjectResponseQuery;
...
export interface QueryEventsParams {
	 ...
	 */
	query: RpcTypes.MySoEventFilter;
	/** optional paging cursor */
...
export type QueryTransactionBlocksParams = {
	...
	order?: 'ascending' | 'descending' | null | undefined;
	signal?: AbortSignal;
} & RpcTypes.MySoTransactionBlockResponseQuery;
...
export interface SubscribeEventParams {
	 ...
	 */
	filter: RpcTypes.MySoEventFilter;
	signal?: AbortSignal;
}
...
export interface UnsafeBatchTransactionParams {
	...
	txnBuilderMode?: RpcTypes.MySoTransactionBlockBuilderMode | null | undefined;
	signal?: AbortSignal;
}
...
export interface UnsafeMoveCallParams {
	...
	/**
	 * the arguments to be passed into the Move function, in [MySoJson](https://docs.mysocial.network/build/myso-json)
	 * format
	 */
	arguments: unknown[];
	/**
	 ...
	 * `MySoTransactionBlockBuilderMode::Commit` when it's None.
	 */
	executionMode?: RpcTypes.MySoTransactionBlockBuilderMode | null | undefined;
	signal?: AbortSignal;
}
/**
...
/**
 * Send all MYSO coins to one recipient. This is for MYSO coin only and does not require a separate gas
 * coin object. Specifically, what pay_all_myso does are: 1. accumulate all MYSO from input coins and
 * deposit all MYSO to the first input coin 2. transfer the updated first coin to the recipient and also
 ...
 */
export interface UnsafePayAllMySoParams {
	/** the transaction signer's MySo address */
	signer: string;
...
/**
 * Send MYSO coins to a list of addresses, following a list of amounts. This is for MYSO coin only and
 * does not require a separate gas coin object. Specifically, what pay_myso does are: 1. debit each
 * input_coin to create new coin following the order of amounts and assign it to the corresponding
 * recipient. 2. accumulate all residual MYSO from input coins left and deposit all MYSO to the first
 ...
 */
export interface UnsafePayMySoParams {
	/** the transaction signer's MySo address */
	signer: string;
	/** the MySo coins to be used in this transaction, including the coin for gas payment. */
	inputCoins: string[];
...
/**
 * Create an unsigned transaction to send MYSO coin object to a MySo address. The MYSO object is also used
 * as the gas object.
 */
export interface UnsafeTransferMySoParams {
	/** the transaction signer's MySo address */
	signer: string;
