	...
	async create(): Promise<RegistrationCredential> {
		return (await navigator.credentials.create({
			publicKey: {
				...
				challenge: new TextEncoder().encode('Create passkey wallet on MySo'),
...
/**
 * @experimental
 * A passkey signer used for signing transactions. This is a client side implementation for [SIP-9](https://github.com/myso-foundation/sips/blob/main/sips/sip-9.md).
 */
export class PasskeyKeypair extends Signer {
	 ...
	 * ```
	 * let provider = new BrowserPasskeyProvider('MySo Passkey Example',{
	 * 	  rpName: 'MySo Passkey Example',
	...
	async sign(data: Uint8Array) {
		...
		// serialize all fields into a passkey signature according to https://github.com/myso-foundation/sips/blob/main/sips/sip-9.md#signature-encoding
