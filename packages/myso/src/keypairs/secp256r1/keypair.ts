...
import { decodeMySoPrivateKey, encodeMySoPrivateKey, Keypair } from '../../cryptography/keypair.js';
...
export class Secp256r1Keypair extends Keypair {
	...
	static fromSecretKey(
		...
		if (typeof secretKey === 'string') {
			const decoded = decodeMySoPrivateKey(secretKey);
		...
		if (!options || !options.skipValidation) {
			...
			const signData = encoder.encode('myso validation');
	 ...
	 */
	getSecretKey(): string {
		return encodeMySoPrivateKey(this.keypair.secretKey, this.getKeyScheme());
	}
