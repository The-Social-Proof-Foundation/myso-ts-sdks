...
export class Transaction {
				...
				: this.#addInput(
						...
						typeof value === 'string'
							? {
									...
									UnresolvedObject: { objectId: normalizeMySoAddress(value) },
								}
							: value,
					);
		},
	 ...
	 * @param options.type - The balance type (e.g., "0x2::myso::MYSO"). Defaults to MYSO.
	 */
	withdrawal({ amount, type }: { amount: number | bigint | string; type?: string | null }): {
		...
		const input: CallArg = {
			...
			FundsWithdrawal: {
				...
				typeArg: { $kind: 'Balance', Balance: type ?? '0x2::myso::MYSO' },
				withdrawFrom:
					// fromSponsor === true
