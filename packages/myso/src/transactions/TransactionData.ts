...
function prepareMySoAddress(address: string) {
	return normalizeMySoAddress(address).replace('0x', '');
}
...
export class TransactionDataBuilder implements TransactionData {
	...
	build({
		...
		const transactionData = {
			sender: prepareMySoAddress(sender),
			expiration: expiration ? expiration : { None: true },
			gasData: {
				payment: gasData.payment,
				owner: prepareMySoAddress(this.gasData.owner ?? sender),
			...
			switch (input.$kind) {
				...
				case 'UnresolvedObject':
					...
					if (
						...
						if (
							normalizeMySoAddress(original.objectId) !== normalizeMySoAddress(resolved.objectId) ||
