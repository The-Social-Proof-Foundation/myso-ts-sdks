...
it('can serialize simplified programmable call struct', () => {
	...
	expect(normalizeMySoAddress(result.package)).toEqual(normalizeMySoAddress(moveCall.package));
	expect(result.typeArguments[0]).toEqual(moveCall.typeArguments[0]);
});

function ref(): { objectId: string; version: string; digest: string } {
	return {
		objectId: normalizeMySoAddress((Math.random() * 100000).toFixed(0).padEnd(64, '0')),
...
it('can serialize transaction data with a programmable transaction', () => {
	const myso = normalizeMySoAddress('0x2');
	const txData = {
		$kind: 'V1',
		V1: {
			sender: normalizeMySoAddress('0xBAD'),
