...
import type { MySoObjectRef } from '../../bcs/types.js';
...
export interface ParallelTransactionExecutorCoinOptions extends ParallelTransactionExecutorBaseOptions {
	...
	/** An initial list of coins used to fund the gas pool, uses all owned MYSO coins by default */
	sourceCoins?: string[];
}
...
export class ParallelTransactionExecutor {
	...
	#defaultGasBudget: bigint;
	#maxPoolSize: number;
	#sourceCoins: Map<string, MySoObjectRef | null> | null;
	...
	async executeTransaction<Include extends MySoClientTypes.TransactionInclude = {}>(
		transaction: Transaction,
		include?: Include,
		additionalSignatures: string[] = [],
	): Promise<MySoClientTypes.TransactionResult<Include & { effects: true }>> {
		const { promise, resolve, reject } =
			promiseWithResolvers<MySoClientTypes.TransactionResult<Include & { effects: true }>>();
	...
	async #execute<Include extends MySoClientTypes.TransactionInclude = {}>(
		transaction: Transaction,
		...
		try {
			transaction.setSenderIfNotSet(this.#signer.toMySoAddress());
			...
			if (gasCoin && gasUsed && gasObject) {
				...
				if (gasOwner === this.#signer.toMySoAddress()) {
					const totalUsed =
			...
			return results as MySoClientTypes.TransactionResult<Include & { effects: true }>;
		} catch (error) {
			if (gasCoin) {
				if (!this.#sourceCoins) {
					this.#sourceCoins = new Map();
	...
	async #refillCoinPool() {
		...
		const address = this.#signer.toMySoAddress();
		...
		if (this.#sourceCoins) {
			...
			if (ids.length > 0) {
				...
				refs.push(
					...objects
						.filter((obj): obj is MySoClientTypes.Object => !(obj instanceof Error))
						.map((obj) => ({
