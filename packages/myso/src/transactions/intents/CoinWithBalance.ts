...
import { normalizeStructTag } from '../../utils/myso-types.js';
...
const MYSO_TYPE = normalizeStructTag('0x2::myso::MYSO');
	...
	return (tx: Transaction) => {
		...
		coinResult = tx.add(
			TransactionCommands.Intent({
				...
				inputs: {},
				data: {
					type: coinType === MYSO_TYPE && useGasCoin ? 'gas' : coinType,
	...
	for (const [index, transaction] of transactionData.commands.entries()) {
		...
		if (balance === 0n) {
			transactionData.replaceCommand(
				...
				TransactionCommands.MoveCall({
					...
					typeArguments: [type === 'gas' ? MYSO_TYPE : type],
				}),
			);
			continue;
		}
		...
		if (addressBalanceByType.get(type)! >= totalByType.get(type)!) {
					...
					arguments: [
						transactionData.addInput(
							...
							Inputs.FundsWithdrawal({
								...
								typeArg: {
									$kind: 'Balance',
									Balance: type === 'gas' ? MYSO_TYPE : type,
								},
								withdrawFrom: {
									$kind: 'Sender',
									Sender: true,
			...
			if (!mergedCoins.has(type)) {
				...
				const coinType = type === 'gas' ? MYSO_TYPE : type;
...
async function getCoinsAndBalanceOfType({
	...
	coins: MySoClientTypes.Coin[];
	balance: bigint;
	addressBalance: bigint;
	coinBalance: bigint;
}> {
	let remainingBalance = balance;
	const coins: MySoClientTypes.Coin[] = [];
	...
	async function loadMoreCoins(cursor: string | null = null): Promise<MySoClientTypes.Coin[]> {
		const {
			objects,
			hasNextPage,
			cursor: nextCursor,
