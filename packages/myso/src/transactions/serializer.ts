...
import { MOVE_STDLIB_ADDRESS, MYSO_FRAMEWORK_ADDRESS } from '../utils/index.js';
...
export function isTxContext(param: MySoClientTypes.OpenSignature): boolean {
	...
	return (
		normalizeMySoAddress(pkg) === MYSO_FRAMEWORK_ADDRESS &&
		module === 'tx_context' &&
		name === 'TxContext'
	);
}
...
export function getPureBcsSchema(
	typeSignature: MySoClientTypes.OpenSignatureBody,
): BcsType<any> | null {
	switch (typeSignature.$kind) {
		...
		case 'datatype': {
			...
			const normalizedPkg = normalizeMySoAddress(pkg);
			...
			if (normalizedPkg === MYSO_FRAMEWORK_ADDRESS) {
