...
export class ZkLoginPublicIdentifier extends PublicKey {
	...
	static fromBytes(
		...
		if (legacyAddress === true) {
			...
			if (publicKey.toMySoAddress() !== address) {
				publicKey = new ZkLoginPublicIdentifier(normalizeZkLoginPublicKeyBytes(bytes, true), {
					client,
				});
			}
		...
		if (address && publicKey.toMySoAddress() !== address) {
	...
	static fromProof(address: string, proof: ZkLoginSignatureInputs) {
		...
		if (legacyPublicKey.toMySoAddress() === address) {
	...
	#toLegacyAddress() {
		...
		return normalizeMySoAddress(
			bytesToHex(blake2b(addressBytes, { dkLen: 32 })).slice(0, MYSO_ADDRESS_LENGTH * 2),
		);
	}
	...
	/**
	 * Return the MySo address associated with this ZkLogin public identifier
	 */
	flag(): number {
	...
	verifyPersonalMessage(message: Uint8Array, signature: Uint8Array | string): Promise<boolean> {
		...
		const address = new ZkLoginPublicIdentifier(parsedSignature.publicKey).toMySoAddress();
	...
	override verifyAddress(address: string): boolean {
		return address === super.toMySoAddress() || address === this.#toLegacyAddress();
	}
}
...
async function graphqlVerifyZkLoginSignature({
	...
	if (!client) {
		throw new Error(
			'A MySo Client (GRPC, GraphQL, or JSON RPC) is required to verify zkLogin signatures',
		);
	}
