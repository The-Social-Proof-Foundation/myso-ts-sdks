...
describe('Balance Withdrawal', () => {
	...
	it('deposits MYSO to address balance and withdraws it', async () => {
		const depositAmount = 100_000_000n; // 0.1 MYSO
	...
	it('uses address balance for gas payment and withdrawal', async () => {
		const depositAmount = 200_000_000n; // 0.2 MYSO (enough for gas + withdrawal)
		const withdrawalAmount = 50_000_000n; // 0.05 MYSO

		// Step 1: Deposit MYSO to address balance
		const depositTx = new Transaction();
		const [coinToDeposit] = depositTx.splitCoins(depositTx.gas, [depositAmount]);
		...
		const [genesisCheckpoint, gasPrice, systemState] = await Promise.all([
			...
			toolbox.jsonRpcClient.getLatestMySoSystemState(),
		]);
		...
		tx.setGasBudget(100_000_000n); // 0.1 MYSO budget
		tx.setExpiration({
			ValidDuring: {
