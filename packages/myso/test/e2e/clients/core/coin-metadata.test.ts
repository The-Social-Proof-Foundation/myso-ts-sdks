...
import { MYSO_TYPE_ARG } from '../../../../src/utils/index.js';
...
describe('Core API - Coin Metadata', () => {
	...
	describe('getCoinMetadata', () => {
		it('all clients return same data for MYSO metadata', async () => {
			await toolbox.expectAllClientsReturnSameData((client) =>
				client.core.getCoinMetadata({ coinType: MYSO_TYPE_ARG }),
			);
		});
		...
		testWithAllClients('should get metadata for MYSO coin', async (client) => {
			...
			expect(result.coinMetadata!.name).toBe('MySo');
			expect(result.coinMetadata!.symbol).toBe('MYSO');
