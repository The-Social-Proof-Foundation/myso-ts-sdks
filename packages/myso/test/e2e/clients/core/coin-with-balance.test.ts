...
import { normalizeMySoAddress, normalizeStructTag } from '../../../../src/utils/index.js';
...
describe('coinWithBalance', () => {
	...
	beforeAll(async () => {
		...
		testType = normalizeMySoAddress(packageId) + '::test::TEST';
		testTypeZero = normalizeMySoAddress(packageId) + '::test_zero::TEST_ZERO';
	...
	testWithAllClients('works with myso', async (client) => {
		...
		tx.transferObjects(
			...
			receiver.toMySoAddress(),
		);
		tx.setSender(publishToolbox.keypair.toMySoAddress());
		...
		expect(
			...
			inputs: [
				{
					Pure: {
						bytes: toBase64(fromHex(receiver.toMySoAddress())),
					},
				},
			],
			sender: publishToolbox.keypair.toMySoAddress(),
			commands: [
				{
					$Intent: {
						data: {
		...
		expect(
			result.balanceChanges?.find(
				(change) =>
					...
					change.owner.AddressOwner === receiver.toMySoAddress(),
			),
		).toEqual({
			amount: '12345',
			coinType: '0x2::myso::MYSO',
			owner: {
				AddressOwner: receiver.toMySoAddress(),
			},
		});
	});
	...
	testWithAllClients('works with custom coin', async (client) => {
		...
		expect(
			result.Transaction!.balanceChanges?.find(
				(change) => change.address === receiver.toMySoAddress(),
			),
		).toEqual({
			amount: '1',
			coinType: testType,
			address: receiver.toMySoAddress(),
		});
	});
		...
		expect(
			result.Transaction?.effects.changedObjects?.filter((change) => {
				...
				return (
					...
					change.outputOwner.AddressOwner === receiver.toMySoAddress()
				);
			}).length,
		).toEqual(1);
	});
	...
	testWithAllClients('works with multiple coins', async (client) => {
		...
		expect(
			...
			{
				amount: '7',
				coinType: normalizeStructTag('0x2::myso::MYSO'),
				address: receiver.toMySoAddress(),
			},
			{
				amount: '3',
	...
	describe('with address balance', () => {
		testWithAllClients('uses address balance for MYSO when available', async (client) => {
			...
			expect(resolved.inputs).toEqual([
				...
				},
				{
					FundsWithdrawal: {
						...
						typeArg: {
							$kind: 'Balance',
							Balance: normalizeStructTag('0x2::myso::MYSO'),
						},
						withdrawFrom: {
							$kind: 'Sender',
							Sender: true,
			...
			expect(resolved.commands).toEqual([
				{
					MoveCall: {
						package: normalizeMySoAddress('0x2'),
						module: 'coin',
						function: 'redeem_funds',
						typeArguments: [normalizeStructTag('0x2::myso::MYSO')],
						arguments: [{ Input: 1 }],
					},
				},
				{
