...
describe('Core API - Name Service', () => {
	...
	describe('defaultNameServiceName', () => {
		// Note: MySoNS is not configured on localnet, so we expect specific behavior
		// gRPC throws "MySoNS not configured for this network"
		...
		testWithAllClients('should handle name service request', async (client) => {
			try {
				...
				// On localnet without MySoNS, name will be null or undefined
				if (result.data.name !== null && result.data.name !== undefined) {
					expect(typeof result.data.name).toBe('string');
				}
			} catch (error) {
				// gRPC and GraphQL may throw errors when MySoNS is not configured
		...
		testWithAllClients('should handle address without name', async (client) => {
			...
			const emptyAddress = new Ed25519Keypair().toMySoAddress();
