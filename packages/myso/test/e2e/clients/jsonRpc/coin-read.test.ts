...
describe('CoinRead API', () => {
	...
	it('Get coins with/without type', async () => {
		const mysoCoins = await toolbox.jsonRpcClient.getCoins({
			owner: toolbox.address(),
		});
		expect(mysoCoins.data.length).toEqual(5);
		...
		const someMySoCoins = await toolbox.jsonRpcClient.getCoins({
			owner: toolbox.address(),
			limit: 3,
		});
		expect(someMySoCoins.data.length).toEqual(3);
		expect(someMySoCoins.nextCursor).toBeTruthy();
	});

	it('Get balance with/without type', async () => {
		const mysoBalance = await toolbox.jsonRpcClient.getBalance({
			owner: toolbox.address(),
		});
		expect(mysoBalance.coinType).toEqual('0x2::myso::MYSO');
		expect(mysoBalance.coinObjectCount).toEqual(5);
		expect(Number(mysoBalance.totalBalance)).toBeGreaterThan(0);
	...
	describe('Address Balance', () => {
		it('filters out fake address balance coins from listCoins', async () => {
			// Deposit some MYSO to the address balance
