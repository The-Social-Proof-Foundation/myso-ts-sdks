...
import { decodeMySoPrivateKey } from '../../src/cryptography/index.js';
...
import { DEFAULT_RECIPIENT, execMySoTools, setup, setupWithFundedAddress } from './utils/setup.js';
...
describe('MultiSig with zklogin signature', () => {
	it('Execute tx with multisig with 1 sig and 1 zkLogin sig combined', async () => {
		...
		const epoch = await tempToolbox.jsonRpcClient.getLatestMySoSystemState();
		const currentEpoch = Number(epoch.epoch);
		const maxEpoch = currentEpoch + 10;

		// Generate a zkLogin signature dynamically using myso keytool
		// This creates a fresh signature with a valid max epoch
		const pmResult = await execMySoTools([
			'myso',
			'keytool',
		...
		const multisigAddr = multiSigPublicKey.toMySoAddress();
		...
		// This is the default ephemeral keypair used by the myso keytool
		const parsed = decodeMySoPrivateKey(
