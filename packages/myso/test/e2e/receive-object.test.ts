...
import { ClientWithCoreApi, MySoClientTypes } from '../../src/client/index.js';

function getOwnerAddress(o: MySoClientTypes.ChangedObject): string | undefined {
	return (
...
describe('Transfer to Object', () => {
	...
	let parentObjectId: MySoClientTypes.ChangedObject;
	let receiveObjectId: MySoClientTypes.ChangedObject;
	...
	beforeEach(async () => {
		...
		const y = x.Transaction?.effects?.changedObjects
			.filter((o: MySoClientTypes.ChangedObject) => o.idOperation === 'Created')
			.map((o: MySoClientTypes.ChangedObject) => getOwnerAddress(o))!;
		receiveObjectId = x.Transaction?.effects?.changedObjects!.filter(
			(o: MySoClientTypes.ChangedObject) =>
		...
		const sharedObject = x.Transaction?.effects?.changedObjects!.filter(
			(o: MySoClientTypes.ChangedObject) => getOwnerAddress(o) === undefined,
		)[0]!;
