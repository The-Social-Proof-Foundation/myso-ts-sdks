...
import { MySoObjectChangeCreated } from '../../src/jsonRpc/index.js';
import type { Keypair } from '../../src/cryptography/index.js';
import { Transaction } from '../../src/transactions/index.js';
import { normalizeMySoObjectId, MYSO_SYSTEM_STATE_OBJECT_ID } from '../../src/utils/index.js';
import {
	DEFAULT_GAS_BUDGET,
	DEFAULT_RECIPIENT,
	publishPackage,
...
export const MYSO_CLOCK_OBJECT_ID = normalizeMySoObjectId('0x6');
...
describe('Transaction Builders', () => {
	...
	it(
		...
		async () => {
			...
			const [{ mysoAddress: validatorAddress }] = await toolbox.getActiveValidators();

			const tx = new Transaction();
			tx.moveCall({
				target: '0x3::myso_system::request_add_stake',
				arguments: [
					tx.object(MYSO_SYSTEM_STATE_OBJECT_ID),
	...
	it('immutable clock', async () => {
		...
		tx.moveCall({
			...
			arguments: [tx.object(MYSO_CLOCK_OBJECT_ID)],
		});
		await validateTransaction(toolbox.grpcClient, toolbox.keypair, tx);
	});
	...
	it('builds object options', async () => {
		...
		tx.moveCall({
			...
			typeArguments: ['0x2::coin::Coin<0x2::myso::MYSO>'],
			arguments: [
				tx.object.option({
		...
		tx.transferObjects([coin3], toolbox.keypair.toMySoAddress());
...
async function validateTransaction(client: ClientWithCoreApi, signer: Keypair, tx: Transaction) {
	tx.setSenderIfNotSet(signer.getPublicKey().toMySoAddress());
