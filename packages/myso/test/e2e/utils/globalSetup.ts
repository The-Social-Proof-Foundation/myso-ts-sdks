...
export default async function setup(project: TestProject) {
	...
	const localnet = await new GenericContainer(`mysten/myso-tools:${MYSO_TOOLS_TAG}`)
		// .withPullPolicy(PullPolicy.alwaysPull())
		.withCommand([
			'myso',
			'start',
			...
			'--force-regenesis',
			'--with-graphql',
			`--with-indexer=postgres://postgres:postgrespw@${pg.getIpAddress(network.getName())}:5432/myso_indexer_v2`,
		])
	...
	project.provide('mysoToolsContainerId', containerId);
