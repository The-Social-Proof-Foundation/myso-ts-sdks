...
import type { MySoObjectChangePublished } from '../../../src/jsonRpc/index.js';
import {
...
import { MYSO_TYPE_ARG, normalizeMySoAddress } from '../../../src/utils/index.js';
...
const MYSO_TOOLS_CONTAINER_ID = inject('mysoToolsContainerId');
...
export class TestToolbox {
	...
	constructor(keypair: Ed25519Keypair, url: string = DEFAULT_FULLNODE_URL, configPath: string) {
		...
		this.jsonRpcClient = new MySoJsonRpcClient({
			network: 'localnet',
	...
	async getGasObjectsOwnedByAddress() {
		return await this.jsonRpcClient.getCoins({
			owner: this.address(),
			coinType: MYSO_TYPE_ARG,
		});
	}

	public async getActiveValidators() {
		return (await this.jsonRpcClient.getLatestMySoSystemState()).activeValidators;
	}
...
export function getClient(url = DEFAULT_FULLNODE_URL): MySoJsonRpcClient {
...
export async function setup(options: { graphQLURL?: string; rpcURL?: string } = {}) {
	...
	await execMySoTools(['mkdir', '-p', configDir]);
...
export async function setupWithFundedAddress(
	...
	const faucetResponse = await retry(
		async () => await requestMySoFromFaucetV2({ host: DEFAULT_FAUCET_URL, recipient: address }),
		{
			backoff: 'EXPONENTIAL',
	...
	await execMySoTools(['myso', 'client', '--yes', '--client.config', configPath]);
...
export async function publishPackage(packageName: string, toolbox?: TestToolbox) {
	...
	return await retry(
		async () => {
			const result = await execMySoTools([
				'myso',
				'move',
				'--client.config',
				toolbox.configPath,
			...
			const packageId = normalizeMySoAddress(
				((publishTxn.objectChanges?.filter(
					(a) => a.type === 'published',
				) as MySoObjectChangePublished[]) ?? [])[0]?.packageId,
			);
...
export async function upgradePackage(
	...
	const { stdout } = await execMySoTools([
		'myso',
		'move',
		'--client.config',
		toolbox.configPath,
	...
	coinId =
		...
		(
			await client.getCoins({
				owner: signer.getPublicKey().toMySoAddress(),
...
export async function executePayMySoNTimes(
	client: MySoJsonRpcClient,
	signer: Keypair,
	nTimes: number,
	...
	for (let i = 0; i < nTimes; i++) {
		...
		txns.push(await payMySo(client, signer, numRecipientsPerTxn, recipients, amounts));
	}
	return txns;
}
...
export async function execMySoTools(
	command: string[],
	options?: Parameters<ContainerRuntimeClient['container']['exec']>[2],
) {
	const container = client.container.getById(MYSO_TOOLS_CONTAINER_ID);
