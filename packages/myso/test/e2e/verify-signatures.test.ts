	...
	describe('personal message signatures', () => {
		describe('single signatures', () => {
			describe('Ed25519', () => {
				...
				it('verifies valid signatures', async () => {
					...
					expect(publicKey.toMySoAddress()).toBe(address);
				});
				...
				it('fails for wrong address', async () => {
					const { signature } = await keypair.signPersonalMessage(message);
					const wrongAddress = new Ed25519Keypair().getPublicKey().toMySoAddress();
					await expect(
			...
			describe('Secp256k1', () => {
				...
				it('fails for wrong address', async () => {
					const { signature } = await keypair.signPersonalMessage(message);
					const wrongAddress = new Secp256k1Keypair().getPublicKey().toMySoAddress();
					await expect(
						verifyPersonalMessageSignature(message, signature, { address: wrongAddress }),
					).rejects.toThrow();
				});
			...
			describe('Secp256r1', () => {
				...
				it('fails for wrong address', async () => {
					const { signature } = await keypair.signPersonalMessage(message);
					const wrongAddress = new Secp256r1Keypair().getPublicKey().toMySoAddress();
					await expect(
						verifyPersonalMessageSignature(message, signature, { address: wrongAddress }),
					).rejects.toThrow();
				});
		...
		describe('multisig signatures', () => {
			...
			it('verifies valid multisig signatures', async () => {
				...
				expect(publicKey.toMySoAddress()).toBe(multiSigPublicKey.toMySoAddress());
			});
		...
		describe('zkLogin signatures', () => {
			const client = new MySoGraphQLClient({ url: DEFAULT_GRAPHQL_URL, network: 'localnet' });
			// this test assumes the localnet epoch is smaller than 3. it will fail if localnet has ran for too long and passed epoch 3.
			// test case generated from `myso keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 3`
			...
			it('verifies valid signatures', async () => {
				...
				expect(publicKey.toMySoAddress()).toBe(
