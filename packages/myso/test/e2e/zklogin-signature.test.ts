...
import { execMySoTools, setup } from './utils/setup.js';
...
describe('zkLogin signature', () => {
	...
	it(
		...
		async () => {
			...
			const epoch = await toolbox.jsonRpcClient.getLatestMySoSystemState();
			...
			const largePmResult = await execMySoTools([
				'myso',
				'keytool',
				'zk-login-insecure-sign-personal-message',
				'--data',
	...
	it(
		...
		async () => {
			...
			// 2. Using execMySoTools to sign it with zklogin
