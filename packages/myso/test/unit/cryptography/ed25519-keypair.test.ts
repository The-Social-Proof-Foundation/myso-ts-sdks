...
import { decodeMySoPrivateKey } from '../../../src/cryptography/keypair.js';
...
// Test case generated against rust keytool cli. See https://github.com/the-social-proof-foundation/myso-core/blob/edd2cd31e0b05d336b1b03b6e79a67d8dd00d06b/crates/myso/src/unit_tests/keytool_tests.rs#L165
const TEST_CASES = [
	[
...
describe('ed25519-keypair', () => {
	...
	it('create keypair from secret key and mnemonics matches keytool', () => {
		for (const t of TEST_CASES) {
			...
			expect(keypair.getPublicKey().toMySoAddress()).toEqual(t[2]);

			// Decode MySo private key from Bech32 string
			const parsed = decodeMySoPrivateKey(t[1]);
			const kp = Ed25519Keypair.fromSecretKey(parsed.secretKey);
			expect(kp.getPublicKey().toMySoAddress()).toEqual(t[2]);
	...
	it('signs Transactions', async () => {
		...
		tx.setSender(keypair.getPublicKey().toMySoAddress());
		tx.setGasPrice(5);
		tx.setGasBudget(100);
		tx.setGasPayment([
			{
