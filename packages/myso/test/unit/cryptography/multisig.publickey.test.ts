...
import { normalizeMySoAddress } from '../../../src/utils/myso-types.js';
...
describe('Publickey', () => {
	...
	it('`toMySoAddress()` should return correct myso address associated with multisig publickey', async () => {
		const multiSigPublicKey = MultiSigPublicKey.fromPublicKeys({
			threshold: 3,
			publicKeys: [
				{ publicKey: pk1, weight: 1 },
		...
		for (const { publicKey, weight } of multiSigPublicKey.getPublicKeys()) {
			const bytes = publicKey.toMySoBytes();
			tmp.set(bytes, i);
			i += bytes.length;
			tmp.set([weight], i++);
		}
		const multisigMySoAddress = normalizeMySoAddress(
			bytesToHex(blake2b(tmp.slice(0, i), { dkLen: 32 })),
		);

		expect(multiSigPublicKey.toMySoAddress()).toEqual(multisigMySoAddress);
		expect(multiSigPublicKey.toMySoAddress()).toEqual(
