...
describe('Multisig scenarios', () => {
	it('multisig address creation and combine sigs using Secp256r1Keypair', async () => {
		...
		tx.setSender(k3.getPublicKey().toMySoAddress());
		tx.setGasPrice(5);
		tx.setGasBudget(100);
		tx.setGasPayment([
			{
...
describe('Multisig address creation:', () => {
	...
	it('`toMultiSigAddress()` with zklogin identifiers', async () => {
		...
		const multisigAddress = multisigPublicKey.toMySoAddress();
...
describe('MultisigKeypair', () => {
	test('signTransaction', async () => {
		...
		expect(signerPubKey.toMySoAddress()).toEqual(publicKey.toMySoAddress());
		...
		expect(signerPubKey2.toMySoAddress()).toEqual(publicKey.toMySoAddress());
	...
	test('signPersonalMessage', async () => {
		...
		expect(signerPubKey3.toMySoAddress()).toEqual(publicKey.toMySoAddress());
	...
	test('unknown signers', async () => {
		...
		expect(() => publicKey.getSigner(k2)).toThrow(
			new Error(`Signer ${pk2.toMySoAddress()} is not part of the MultiSig public key`),
		);
	});
});
