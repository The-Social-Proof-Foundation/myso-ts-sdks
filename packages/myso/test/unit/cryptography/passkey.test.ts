...
describe('passkey signer E2E testing', () => {
	...
	it('should verify a transaction from rust implementation', async () => {
		// generated test vector from `test_passkey_authenticator` in crates/myso-types/src/unit_tests/passkey_authenticator_test.rs
	...
	it('should verify a transaction from a real passkey output', async () => {
		// generated test vector from a real iphone passkey output from broswer app: https://github.com/joyqvq/myso-webauthn-poc
		const sig = fromBase64(
	...
	it('should sign and recover to an unique public key', async () => {
		...
		expect(signer2.getPublicKey().toMySoAddress()).toEqual(address);
	});
});
