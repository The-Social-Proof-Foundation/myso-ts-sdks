...
import { normalizeMySoAddress, MYSO_ADDRESS_LENGTH } from '../../../src/utils/myso-types.js';
...
describe('Publickey', () => {
	...
	it('`toMySoPublicKey()` should return a valid myso representation', async () => {
		expect(pk2.toMySoPublicKey()).toEqual(toBase64(pk2.toMySoBytes()));
		expect(pk2.toMySoPublicKey()).toEqual('AQIdFSMHxrcrDtBBiw5wzYDn9SlbjYb1ci0/UhP70jlPNg==');
	});
	...
	it('`toMySoBytes()` should return the correct byte representation of the public key with the signature scheme flag', async () => {
		const pk1MySoBytes = new Uint8Array(pk1.toRawBytes().length + 1);
		pk1MySoBytes.set([0x00]);
		pk1MySoBytes.set(pk1.toRawBytes(), 1);

		expect(pk1.toMySoBytes()).toEqual(pk1MySoBytes);
		expect(pk1.toMySoBytes()).toEqual(
			new Uint8Array([
		...
		const pk2MySoBytes = new Uint8Array(pk2.toRawBytes().length + 1);
		pk2MySoBytes.set([0x01]);
		pk2MySoBytes.set(pk2.toRawBytes(), 1);

		expect(pk2.toMySoBytes()).toEqual(pk2MySoBytes);
		expect(pk2.toMySoBytes()).toEqual(
			new Uint8Array([
		...
		const pk3MySoBytes = new Uint8Array(pk3.toRawBytes().length + 1);
		pk3MySoBytes.set([0x02]);
		pk3MySoBytes.set(pk3.toRawBytes(), 1);

		expect(pk3.toMySoBytes()).toEqual(pk3MySoBytes);
		expect(pk3.toMySoBytes()).toEqual(
			new Uint8Array([
	...
	it('`toMySoAddress()` should correctly return myso address associated with Ed25519 publickey', async () => {
		const pk1MySoAddress = normalizeMySoAddress(
			bytesToHex(blake2b(pk1.toMySoBytes(), { dkLen: 32 })).slice(0, MYSO_ADDRESS_LENGTH * 2),
		);
		const pk2MySoAddress = normalizeMySoAddress(
			bytesToHex(blake2b(pk2.toMySoBytes(), { dkLen: 32 })).slice(0, MYSO_ADDRESS_LENGTH * 2),
		);
		const pk3MySoAddress = normalizeMySoAddress(
			bytesToHex(blake2b(pk3.toMySoBytes(), { dkLen: 32 })).slice(0, MYSO_ADDRESS_LENGTH * 2),
		);
		expect(k1.toMySoAddress()).toEqual(pk1MySoAddress);
		expect(k1.toMySoAddress()).toEqual(
			'0xafedf3bc60bd296aa6830d7c48ca44e0f7a32478ae4bd7b9a6ac1dc81ff7b29b',
		);

		expect(k2.toMySoAddress()).toEqual(pk2MySoAddress);
		expect(k2.toMySoAddress()).toEqual(
			'0x7e4f9a35bf3b5383802d990956d6f3c93e6184ebbbcf0820c124ab3a59ef77ac',
		);

		expect(k3.toMySoAddress()).toEqual(pk3MySoAddress);
		expect(k3.toMySoAddress()).toEqual(
