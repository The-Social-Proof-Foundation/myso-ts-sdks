...
describe('zkLogin address', () => {
	...
	test('computeZkLoginAddressFromSeed matches ZkLoginPublicIdentifier.toMySoAddress() for legacy addresses', () => {
		const seed = BigInt(
		...
		expect(computeZkLoginAddressFromSeed(seed, iss, true)).toEqual(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: true }).toMySoAddress(),
		);
	});

	test('computeZkLoginAddressFromSeed matches ZkLoginPublicIdentifier.toMySoAddress() for non-legacy addresses', () => {
		const seed = BigInt(
			'380704556853533152350240698167704405529973457670972223618755249929828551006',
		);
		const iss = 'https://accounts.google.com';
		expect(computeZkLoginAddressFromSeed(seed, iss, false)).toEqual(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: false }).toMySoAddress(),
		);
	});
