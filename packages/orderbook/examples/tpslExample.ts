 ...
 * in the Orderbook margin trading system.
 *
...
import { orderbook, OrderType, SelfMatchingOptions } from '../src/index.js';
...
(async () => {
	...
	try {
		const conditionalOrderIds = await client.orderbook.getConditionalOrderIds('MARGIN_MANAGER_1');
	...
	try {
		const lowestTriggerAbove = await client.orderbook.getLowestTriggerAbovePrice('MARGIN_MANAGER_1');
	...
	try {
		const highestTriggerBelow =
			await client.orderbook.getHighestTriggerBelowPrice('MARGIN_MANAGER_1');
	...
	try {
		const managerState = await client.orderbook.getMarginManagerState('MARGIN_MANAGER_1');
	...
	await client.orderbook.getPriceInfoObject(tx, 'MYSO');
	await client.orderbook.getPriceInfoObject(tx, 'DBUSDC');
	...
	client.orderbook.marginTPSL.addConditionalOrder({
	...
	client.orderbook.marginTPSL.cancelConditionalOrder('MARGIN_MANAGER_1', '1002')(tx);
	...
	client.orderbook.marginTPSL.cancelAllConditionalOrders('MARGIN_MANAGER_1')(tx);
	...
	client.orderbook.marginTPSL.executeConditionalOrders('MARGIN_MANAGER_1', 10)(tx);
	...
	await client.orderbook.getPriceInfoObject(tx2, 'MYSO');
	await client.orderbook.getPriceInfoObject(tx2, 'DBUSDC');

	// 3.1 Create a condition manually
	const condition = client.orderbook.marginTPSL.newCondition(
		'MYSO_DBUSDC', // Pool key for price calculation
		true, // triggerBelowPrice
		4.0, // triggerPrice
	)(tx2);

	// 3.2 Create a pending limit order manually
	const pendingLimitOrder = client.orderbook.marginTPSL.newPendingLimitOrder('MYSO_DBUSDC', {
		clientOrderId: '3001',
	...
	const pendingMarketOrder = client.orderbook.marginTPSL.newPendingMarketOrder('MYSO_DBUSDC', {
		clientOrderId: '3002',
		selfMatchingOption: SelfMatchingOptions.SELF_MATCHING_ALLOWED,
		quantity: 2,
