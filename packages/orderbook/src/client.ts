...
import { OrderbookContract } from './transactions/orderbook.js';
import { OrderbookAdminContract } from './transactions/orderbookAdmin.js';
...
import { MySoPriceServiceConnection } from './pyth/pyth.js';
import { MySoPythClient } from './pyth/pyth.js';
...
export interface OrderbookCompatibleClient extends ClientWithCoreApi {}

export interface OrderbookOptions<Name = 'orderbook'> {
	address: string;
...
/**
 * OrderbookClient class for managing Orderbook operations.
 */
export class OrderbookClient {
	...
	constructor({
		...
		this.orderbookAdmin = new OrderbookAdminContract(this.#config);
	...
	async checkManagerBalanceWithAddress(managerAddress: string, coinKey: string) {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::balance`,
	...
	async whitelisted(poolKey: string) {
		...
		tx.add(this.orderbook.whitelisted(poolKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getQuoteQuantityOut(poolKey: string, baseQuantity: number) {
		...
		tx.add(this.orderbook.getQuoteQuantityOut(poolKey, baseQuantity));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getBaseQuantityOut(poolKey: string, quoteQuantity: number) {
		...
		tx.add(this.orderbook.getBaseQuantityOut(poolKey, quoteQuantity));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getQuantityOut(poolKey: string, baseQuantity: number, quoteQuantity: number) {
		...
		tx.add(this.orderbook.getQuantityOut(poolKey, baseQuantity, quoteQuantity));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async accountOpenOrders(poolKey: string, managerKey: string) {
		...
		tx.add(this.orderbook.accountOpenOrders(poolKey, managerKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getOrder(poolKey: string, orderId: string) {
		...
		tx.add(this.orderbook.getOrder(poolKey, orderId));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getOrders(poolKey: string, orderIds: string[]) {
		...
		tx.add(this.orderbook.getOrders(poolKey, orderIds));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getLevel2Range(poolKey: string, priceLow: number, priceHigh: number, isBid: boolean) {
		...
		tx.add(this.orderbook.getLevel2Range(poolKey, priceLow, priceHigh, isBid));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getLevel2TicksFromMid(poolKey: string, ticks: number) {
		...
		tx.add(this.orderbook.getLevel2TicksFromMid(poolKey, ticks));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async vaultBalances(poolKey: string) {
		...
		tx.add(this.orderbook.vaultBalances(poolKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getPoolIdByAssets(baseType: string, quoteType: string) {
		const tx = new Transaction();
		tx.add(this.orderbook.getPoolIdByAssets(baseType, quoteType));
	...
	async midPrice(poolKey: string) {
		const tx = new Transaction();
		const pool = this.#config.getPool(poolKey);
		tx.add(this.orderbook.midPrice(poolKey));
	...
	async poolTradeParams(poolKey: string) {
		...
		tx.add(this.orderbook.poolTradeParams(poolKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async poolBookParams(poolKey: string) {
		...
		tx.add(this.orderbook.poolBookParams(poolKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async account(poolKey: string, managerKey: string) {
		...
		tx.add(this.orderbook.account(poolKey, managerKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async lockedBalance(poolKey: string, balanceManagerKey: string) {
		...
		tx.add(this.orderbook.lockedBalance(poolKey, balanceManagerKey));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getBalanceManagerIds(owner: string): Promise<string[]> {
		const tx = new Transaction();
		tx.add(this.orderbook.getBalanceManagerIds(owner));
		...
		return vecOfAddresses.map((id: string) => normalizeMySoAddress(id));
	}

	/**
	 * @description Get the owner of the referral (OrderbookPoolReferral)
	...
	async getPoolReferralBalances(
		...
		tx.add(this.orderbook.getPoolReferralBalances(poolKey, referral));
	...
	/**
	 * @description Get the pool ID associated with a referral (OrderbookPoolReferral)
	 * @param {string} referral The referral (OrderbookPoolReferral) to get the pool ID for
	...
	async balanceManagerReferralPoolId(referral: string): Promise<string> {
		...
		return normalizeMySoAddress(poolId);
	}
	...
	async poolReferralMultiplier(poolKey: string, referral: string): Promise<number> {
		...
		tx.add(this.orderbook.poolReferralMultiplier(poolKey, referral));
	...
	async getBalanceManagerReferralId(managerKey: string, poolKey: string): Promise<string | null> {
		...
		try {
			...
			return normalizeMySoAddress(optionId);
		} catch {
			return null;
		}
	}
	...
	async getPriceInfoObject(tx: Transaction, coinKey: string): Promise<string> {
		...
		// Initialize MySo Client and Pyth Client
		const wormholeStateId = this.#config.pyth.wormholeStateId;
		const pythStateId = this.#config.pyth.pythStateId;

		const client = new MySoPythClient(this.#client, pythStateId, wormholeStateId);
	...
	async getPriceInfoObjects(tx: Transaction, coinKeys: string[]): Promise<Record<string, string>> {
		...
		const wormholeStateId = this.#config.pyth.wormholeStateId;
		const pythStateId = this.#config.pyth.pythStateId;
		const pythClient = new MySoPythClient(this.#client, pythStateId, wormholeStateId);
	...
	/**
	 * @description Check if a orderbook pool is allowed for borrowing from margin pool
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {string} orderbookPoolId The ID of the orderbook pool
	 * @returns {Promise<boolean>} Whether the orderbook pool is allowed
	 */
	async isOrderbookPoolAllowed(coinKey: string, orderbookPoolId: string): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.marginPool.orderbookPoolAllowed(coinKey, orderbookPoolId));
	...
	async getMarginManagerOwner(marginManagerKey: string): Promise<string> {
		...
		const bytes = res.commandResults![0].returnValues[0].bcs;
		return normalizeMySoAddress(bcs.Address.parse(bytes));
	}

	/**
	 * @description Get the Orderbook pool ID associated with a margin manager
	 * @param {string} marginManagerKey The key to identify the margin manager
	 * @returns {Promise<string>} The Orderbook pool ID
	 */
	async getMarginManagerOrderbookPool(marginManagerKey: string): Promise<string> {
		const manager = this.#config.getMarginManager(marginManagerKey);
		const tx = new Transaction();
		tx.add(this.marginManager.orderbookPool(manager.poolKey, manager.address));
	...
	async getMarginManagerMarginPoolId(marginManagerKey: string): Promise<string | null> {
		...
		return option ? normalizeMySoAddress(option) : null;
	}
	...
	async getMarginManagerState(
		...
		const managerId = normalizeMySoAddress(
			bcs.Address.parse(res.commandResults[0].returnValues[0].bcs),
		);
		const orderbookPoolId = normalizeMySoAddress(
	...
	> {
		...
		for (let i = 0; i < entries.length; i++) {
			...
			const managerId = normalizeMySoAddress(bcs.Address.parse(commandResult.returnValues[0].bcs));
			const orderbookPoolId = normalizeMySoAddress(
	...
	async getMarginAccountOrderDetails(marginManagerKey: string) {
		...
		tx.setSenderIfNotSet(normalizeMySoAddress(this.#address));
	...
	/**
	 * @description Check if a orderbook pool is enabled for margin trading
	...
	async getMarginManagerIdsForOwner(owner: string): Promise<string[]> {
		...
		return vecSet.contents.map((id) => normalizeMySoAddress(id));
	}

	/**
	 * @description Get the base margin pool ID for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the quote margin pool ID for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the minimum withdraw risk ratio for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the minimum borrow risk ratio for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the liquidation risk ratio for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the target liquidation risk ratio for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the user liquidation reward for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	/**
	 * @description Get the pool liquidation reward for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	...
	async stablePool(poolKey: string): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.orderbook.stablePool(poolKey));
	...
	async registeredPool(poolKey: string): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.orderbook.registeredPool(poolKey));
	...
	async getQuoteQuantityOutInputFee(poolKey: string, baseQuantity: number) {
		...
		tx.add(this.orderbook.getQuoteQuantityOutInputFee(poolKey, baseQuantity));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getBaseQuantityOutInputFee(poolKey: string, quoteQuantity: number) {
		...
		tx.add(this.orderbook.getBaseQuantityOutInputFee(poolKey, quoteQuantity));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getQuantityOutInputFee(poolKey: string, baseQuantity: number, quoteQuantity: number) {
		...
		tx.add(this.orderbook.getQuantityOutInputFee(poolKey, baseQuantity, quoteQuantity));
		const res = await this.#client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getAccountOrderDetails(poolKey: string, managerKey: string) {
		const tx = new Transaction();
		tx.add(this.orderbook.getAccountOrderDetails(poolKey, managerKey));
	...
	async accountExists(poolKey: string, managerKey: string): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.orderbook.accountExists(poolKey, managerKey));
	...
	async poolTradeParamsNext(poolKey: string) {
		const tx = new Transaction();
		tx.add(this.orderbook.poolTradeParamsNext(poolKey));
	...
	async quorum(poolKey: string): Promise<number> {
		const tx = new Transaction();
		tx.add(this.orderbook.quorum(poolKey));
	...
	async poolId(poolKey: string): Promise<string> {
		const tx = new Transaction();
		tx.add(this.orderbook.poolId(poolKey));
	...
	async canPlaceLimitOrder(params: CanPlaceLimitOrderParams): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.orderbook.canPlaceLimitOrder(params));
	...
	async canPlaceMarketOrder(params: CanPlaceMarketOrderParams): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.orderbook.canPlaceMarketOrder(params));
	...
	async checkMarketOrderParams(poolKey: string, quantity: number): Promise<boolean> {
		const tx = new Transaction();
		tx.add(this.orderbook.checkMarketOrderParams(poolKey, quantity));
	...
	async checkLimitOrderParams(
		...
		const tx = new Transaction();
		tx.add(this.orderbook.checkLimitOrderParams(poolKey, price, quantity, expireTimestamp));
