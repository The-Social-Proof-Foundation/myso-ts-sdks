 ...
 * `myso::table` in that its keys and values are not stored within the `Bag` value,
 * but instead are stored using MySo's object system. The `Bag` struct acts only as
 ...
 * `myso::dynamic_field` while preventing accidentally stranding field values. A
