...
export class BalanceManagerContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration for BalanceManagerContract
	 */
	...
	createAndShareBalanceManager = () => (tx: Transaction) => {
		const manager = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::new`,
		});

		tx.moveCall({
			target: '0x2::transfer::public_share_object',
			arguments: [manager],
			typeArguments: [`${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::BalanceManager`],
		});
	};
	...
	createBalanceManagerWithOwner = (ownerAddress: string) => (tx: Transaction) => {
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::new_with_custom_owner`,
	...
	depositIntoManager =
		(managerKey: string, coinKey: string, amountToDeposit: number) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::deposit`,
	...
	withdrawFromManager =
		...
		(tx: Transaction) => {
			...
			const coinObject = tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::withdraw`,
	...
	withdrawAllFromManager =
		(managerKey: string, coinKey: string, recipient: string) => (tx: Transaction) => {
			...
			const withdrawalCoin = tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::withdraw_all`,
	...
	generateProofAsOwner = (managerId: string) => (tx: Transaction) => {
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::generate_proof_as_owner`,
			arguments: [tx.object(managerId)],
		});
	};
	...
	generateProofAsTrader = (managerId: string, tradeCapId: string) => (tx: Transaction) => {
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::generate_proof_as_trader`,
	...
	mintTradeCap = (managerKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::mint_trade_cap`,
			arguments: [tx.object(managerId)],
		});
	};
	...
	mintDepositCap = (managerKey: string) => (tx: Transaction) => {
		...
		const managerId = manager.address;
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::mint_deposit_cap`,
			arguments: [tx.object(managerId)],
		});
	};
	...
	mintWithdrawalCap = (managerKey: string) => (tx: Transaction) => {
		...
		const managerId = manager.address;
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::mint_withdraw_cap`,
			arguments: [tx.object(managerId)],
		});
	};
	...
	depositWithCap =
		(managerKey: string, coinKey: string, amountToDeposit: number) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::deposit_with_cap`,
	...
	withdrawWithCap =
		(managerKey: string, coinKey: string, amountToWithdraw: number) => (tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::withdraw_with_cap`,
	 ...
	 * @param {string} managerKey The name of the BalanceManager
	 * @param {string} referral The referral (OrderbookPoolReferral) to set the BalanceManager to
	...
	setBalanceManagerReferral =
		(managerKey: string, referral: string, tradeCap: TransactionArgument) => (tx: Transaction) => {
			const managerId = this.#config.getBalanceManager(managerKey).address;
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::set_balance_manager_referral`,
	...
	unsetBalanceManagerReferral =
		(managerKey: string, poolKey: string, tradeCap: TransactionArgument) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::unset_balance_manager_referral`,
	...
	registerBalanceManager = (managerKey: string) => (tx: Transaction) => {
		const managerId = this.#config.getBalanceManager(managerKey).address;
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::register_balance_manager`,
	...
	owner = (managerKey: string) => (tx: Transaction) => {
		const managerId = this.#config.getBalanceManager(managerKey).address;
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::owner`,
			arguments: [tx.object(managerId)],
		});
	};
	...
	id = (managerKey: string) => (tx: Transaction) => {
		const managerId = this.#config.getBalanceManager(managerKey).address;
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::id`,
			arguments: [tx.object(managerId)],
		});
	};
	...
	balanceManagerReferralOwner = (referralId: string) => (tx: Transaction) => {
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::balance_manager_referral_owner`,
	...
	balanceManagerReferralPoolId = (referralId: string) => (tx: Transaction) => {
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::balance_manager_referral_pool_id`,
			arguments: [tx.object(referralId)],
		});
	};
	...
	getBalanceManagerReferralId = (managerKey: string, poolKey: string) => (tx: Transaction) => {
		...
		const poolId = this.#config.getPool(poolKey).address;
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::get_balance_manager_referral_id`,
	...
	revokeTradeCap = (managerKey: string, tradeCapId: string) => (tx: Transaction) => {
		const managerId = this.#config.getBalanceManager(managerKey).address;
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::balance_manager::revoke_trade_cap`,
