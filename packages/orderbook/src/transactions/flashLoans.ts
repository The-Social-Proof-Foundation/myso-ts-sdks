...
export class FlashLoanContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration object for Orderbook
	 */
	...
	borrowBaseAsset = (poolKey: string, borrowAmount: number) => (tx: Transaction) => {
		...
		const [baseCoinResult, flashLoan] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::borrow_flashloan_base`,
	...
	returnBaseAsset =
		...
		(tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::return_flashloan_base`,
	...
	borrowQuoteAsset = (poolKey: string, borrowAmount: number) => (tx: Transaction) => {
		...
		const [quoteCoinResult, flashLoan] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::borrow_flashloan_quote`,
			arguments: [tx.object(pool.address), tx.pure.u64(inputQuantity)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	...
	returnQuoteAsset =
		...
		(tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::return_flashloan_quote`,
