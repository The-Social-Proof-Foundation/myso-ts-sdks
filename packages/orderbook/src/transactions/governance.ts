...
export class GovernanceContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration for GovernanceContract
	 */
	...
	unstake = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::unstake`,
	...
	submitProposal = (params: ProposalParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::submit_proposal`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
	...
	vote = (poolKey: string, balanceManagerKey: string, proposal_id: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::vote`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
