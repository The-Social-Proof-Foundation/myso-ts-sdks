...
export class MarginAdminContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration for MarginAdminContract
	 */
	...
	/**
	 * @description Register a orderbook pool
	 ...
	 * @returns A function that takes a Transaction object
	 */
	registerOrderbookPool =
		(poolKey: string, poolConfig: TransactionArgument) => (tx: Transaction) => {
			const pool = this.#config.getPool(poolKey);
			const baseCoin = this.#config.getCoin(pool.baseCoin);
			const quoteCoin = this.#config.getCoin(pool.quoteCoin);
			tx.moveCall({
				target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::register_orderbook_pool`,
				arguments: [
					tx.object(this.#config.MARGIN_REGISTRY_ID),
					tx.object(this.#marginAdminCap()),
	...
	/**
	 * @description Enable a orderbook pool for margin trading
	 * @param {string} poolKey The key of the pool to be enabled
	 * @returns A function that takes a Transaction object
	 */
	enableOrderbookPool = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::enable_orderbook_pool`,
			arguments: [
				tx.object(this.#config.MARGIN_REGISTRY_ID),
				tx.object(this.#marginAdminCap()),
				tx.object(pool.address),
	...
	/**
	 * @description Disable a orderbook pool from margin trading
	 * @param {string} poolKey The key of the pool to be disabled
	 * @returns A function that takes a Transaction object
	 */
	disableOrderbookPool = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::disable_orderbook_pool`,
			arguments: [
				tx.object(this.#config.MARGIN_REGISTRY_ID),
				tx.object(this.#marginAdminCap()),
				tx.object(pool.address),
