...
/**
 * OrderbookMaintainerContract class for managing maintainer actions.
 */
export class MarginMaintainerContract {
	#config: OrderbookConfig;

	/**
	 * @param {OrderbookConfig} config Configuration for MarginMaintainerContract
	 */
	...
	/**
	 * @description Enable a orderbook pool for loan
	 * @param {string} orderbookPoolKey The key to identify the orderbook pool
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {TransactionObjectArgument} marginPoolCap The margin pool cap
	 * @returns A function that takes a Transaction object
	 */
	enableOrderbookPoolForLoan =
		(orderbookPoolKey: string, coinKey: string, marginPoolCap: TransactionObjectArgument) =>
		(tx: Transaction) => {
			const orderbookPool = this.#config.getPool(orderbookPoolKey);
			const marginPool = this.#config.getMarginPool(coinKey);
			tx.moveCall({
				target: `${this.#config.MARGIN_PACKAGE_ID}::margin_pool::enable_orderbook_pool_for_loan`,
				arguments: [
					tx.object(marginPool.address),
					tx.object(this.#config.MARGIN_REGISTRY_ID),
					tx.pure.id(orderbookPool.address),
	...
	/**
	 * @description Disable a orderbook pool for loan
	 * @param {string} orderbookPoolKey The key to identify the orderbook pool
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {TransactionObjectArgument} marginPoolCap The margin pool cap
	 * @returns A function that takes a Transaction object
	 */
	disableOrderbookPoolForLoan =
		(orderbookPoolKey: string, coinKey: string, marginPoolCap: TransactionObjectArgument) =>
		(tx: Transaction) => {
			const orderbookPool = this.#config.getPool(orderbookPoolKey);
			const marginPool = this.#config.getMarginPool(coinKey);
			tx.moveCall({
				target: `${this.#config.MARGIN_PACKAGE_ID}::margin_pool::disable_orderbook_pool_for_loan`,
				arguments: [
					tx.object(marginPool.address),
					tx.object(this.#config.MARGIN_REGISTRY_ID),
					tx.pure.id(orderbookPool.address),
