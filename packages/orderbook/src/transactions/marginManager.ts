...
export class MarginManagerContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration for MarginManagerContract
	 */
	...
	/**
	 * @description Set the referral for a margin manager (OrderbookPoolReferral)
	 * @param {string} managerKey The key to identify the margin manager
	 * @param {string} referral The referral (OrderbookPoolReferral) to set
	 * @returns A function that takes a Transaction object
	 */
	 ...
	 * @returns A function that takes a Transaction object
	 */
	orderbookPool = (poolKey: string, marginManagerId: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		return tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_manager::orderbook_pool`,
			arguments: [tx.object(marginManagerId)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	 ...
	 * @param {string} marginManagerId The ID of the margin manager
	 * @returns A function that takes a Transaction object
	 * @returns Returns (manager_id, orderbook_pool_id, risk_ratio, base_asset, quote_asset,
