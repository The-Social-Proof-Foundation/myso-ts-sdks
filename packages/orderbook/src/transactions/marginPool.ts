...
export class MarginPoolContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration for MarginPoolContract
	 */
	...
	/**
	 * @description Check if a orderbook pool is allowed for borrowing
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {string} orderbookPoolId The ID of the orderbook pool
	 * @returns A function that takes a Transaction object
	 */
	orderbookPoolAllowed = (coinKey: string, orderbookPoolId: string) => (tx: Transaction) => {
		const marginPool = this.#config.getMarginPool(coinKey);
		return tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_pool::orderbook_pool_allowed`,
			arguments: [tx.object(marginPool.address), tx.pure.id(orderbookPoolId)],
			typeArguments: [marginPool.type],
		});
	};
