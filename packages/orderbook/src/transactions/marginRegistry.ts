...
export class MarginRegistryContract {
	...
	/**
	 * @param {OrderbookConfig} config Configuration for MarginRegistryContract
	 */
	...
	/**
	 * @description Get the margin pool IDs (base and quote) for a orderbook pool
	 * @param {string} poolKey The key to identify the pool
	 * @returns A function that takes a Transaction object
	 */
	getOrderbookPoolMarginPoolIds = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		return tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::get_orderbook_pool_margin_pool_ids`,
