...
/**
 * OrderbookContract class for managing Orderbook operations.
 */
export class OrderbookContract {
	#config: OrderbookConfig;

	/**
	 * @param {OrderbookConfig} config Configuration for OrderbookContract
	 */
	...
	placeLimitOrder = (params: PlaceLimitOrderParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::place_limit_order`,
			arguments: [
	...
	placeMarketOrder = (params: PlaceMarketOrderParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::place_market_order`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
	...
	modifyOrder =
		...
		(tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::modify_order`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelOrder =
		(poolKey: string, balanceManagerKey: string, orderId: string) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::cancel_order`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelOrders =
		(poolKey: string, balanceManagerKey: string, orderIds: string[]) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::cancel_orders`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelAllOrders = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::cancel_all_orders`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
	...
	withdrawSettledAmounts = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::withdraw_settled_amounts`,
	...
	withdrawSettledAmountsPermissionless =
		(poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::withdraw_settled_amounts_permissionless`,
	...
	/**
	 * @description Add a myso price point for a target pool using a reference pool
	 ...
	 * @returns A function that takes a Transaction object
	 */
	addMySoPricePoint = (targetPoolKey: string, referencePoolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::add_myso_price_point`,
			arguments: [
	...
	claimRebates = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::claim_rebates`,
			arguments: [tx.object(pool.address), tx.object(balanceManager.address), tradeProof],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	mintReferral = (poolKey: string, multiplier: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::mint_referral`,
	...
	/**
	 * @description Update the referral multiplier for a pool (OrderbookPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (OrderbookPoolReferral) to update
	 * @param {number} multiplier The multiplier for the referral
	 * @returns A function that takes a Transaction object
	 */
	updatePoolReferralMultiplier =
		(poolKey: string, referral: string, multiplier: number) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::update_pool_referral_multiplier`,
	...
	/**
	 * @description Claim the rewards for a referral (OrderbookPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (OrderbookPoolReferral) to claim the rewards for
	 * @returns A function that takes a Transaction object
	 */
	claimPoolReferralRewards = (poolKey: string, referral: string) => (tx: Transaction) => {
		...
		const [baseRewards, quoteRewards, mysoRewards] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::claim_pool_referral_rewards`,
			arguments: [tx.object(pool.address), tx.object(referral)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});

		return { baseRewards, quoteRewards, mysoRewards };
	};
	...
	updatePoolAllowedVersions = (poolKey: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::update_pool_allowed_versions`,
	...
	getOrder = (poolKey: string, orderId: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_order`,
	...
	getOrders = (poolKey: string, orderIds: string[]) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_orders`,
	...
	/**
	 * @description Burn MYUSD tokens from the pool
	 * @param {string} poolKey The key to identify the pool
	 * @returns A function that takes a Transaction object
	 */
	burnMySo = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::burn_myso`,
			arguments: [tx.object(pool.address), tx.object(this.#config.MYUSD_TREASURY_ID)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	midPrice = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::mid_price`,
	...
	whitelisted = (poolKey: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::whitelisted`,
	...
	getQuoteQuantityOut = (poolKey: string, baseQuantity: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_quote_quantity_out`,
			arguments: [
				tx.object(pool.address),
	...
	getBaseQuantityOut = (poolKey: string, quoteQuantity: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_base_quantity_out`,
			arguments: [
				tx.object(pool.address),
	...
	getQuantityOut =
		(poolKey: string, baseQuantity: number, quoteQuantity: number) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_quantity_out`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(Math.round(baseQuantity * baseCoin.scalar)),
					tx.pure.u64(Math.round(quoteQuantity * quoteScalar)),
	...
	accountOpenOrders = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::account_open_orders`,
	...
	getLevel2Range =
		(poolKey: string, priceLow: number, priceHigh: number, isBid: boolean) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_level2_range`,
				arguments: [
					tx.object(pool.address),
	...
	getLevel2TicksFromMid = (poolKey: string, tickFromMid: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_level2_ticks_from_mid`,
	...
	vaultBalances = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::vault_balances`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getPoolIdByAssets = (baseType: string, quoteType: string) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_pool_id_by_asset`,
	...
	swapExactBaseForQuote = (params: SwapParams) => (tx: Transaction) => {
		...
		const { poolKey, amount: baseAmount, mysoAmount, minOut: minQuote } = params;

		const pool = this.#config.getPool(poolKey);
		const mysoCoinType = this.#config.getCoin('MYUSD').type;
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		...
		const mysoCoin =
			params.mysoCoin ??
			coinWithBalance({ type: mysoCoinType, balance: Math.round(mysoAmount * MYUSD_SCALAR) });

		const minQuoteInput = Math.round(minQuote * quoteCoin.scalar);

		const [baseCoinResult, quoteCoinResult, mysoCoinResult] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::swap_exact_base_for_quote`,
			arguments: [
				tx.object(pool.address),
				baseCoinInput,
				mysoCoin,
		...
		return [baseCoinResult, quoteCoinResult, mysoCoinResult] as const;
	};
	...
	swapExactQuoteForBase = (params: SwapParams) => (tx: Transaction) => {
		...
		const { poolKey, amount: quoteAmount, mysoAmount, minOut: minBase } = params;
		...
		const [baseCoinResult, quoteCoinResult, mysoCoinResult] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::swap_exact_quote_for_base`,
			arguments: [
				tx.object(pool.address),
				quoteCoinInput,
				mysoCoin,
	...
	swapExactQuantity = (params: SwapParams & { isBaseToCoin: boolean }) => (tx: Transaction) => {
		...
		const { poolKey, amount, mysoAmount, minOut, baseCoin, quoteCoin, mysoCoin, isBaseToCoin } =
			params;
		...
		const [baseCoinResult, quoteCoinResult, mysoCoinResult] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::swap_exact_quantity`,
			arguments: [
				tx.object(pool.address),
				baseCoinInput,
				quoteCoinInput,
	...
	swapExactBaseForQuoteWithManager = (params: SwapWithManagerParams) => (tx: Transaction) => {
		...
		const [baseCoinResult, quoteCoinResult] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::swap_exact_base_for_quote_with_manager`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tx.object(tradeCap),
	...
	swapExactQuoteForBaseWithManager = (params: SwapWithManagerParams) => (tx: Transaction) => {
		...
		const [baseCoinResult, quoteCoinResult] = tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::swap_exact_quote_for_base_with_manager`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tx.object(tradeCap),
	...
	swapExactQuantityWithManager =
		(params: SwapWithManagerParams & { isBaseToCoin: boolean }) => (tx: Transaction) => {
			...
			const [baseCoinResult, quoteCoinResult] = tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::swap_exact_quantity_with_manager`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tx.object(tradeCap),
	...
	createPermissionlessPool = (params: CreatePermissionlessPoolParams) => (tx: Transaction) => {
		tx.setSenderIfNotSet(this.#config.address);
		const { baseCoinKey, quoteCoinKey, tickSize, lotSize, minSize, mysoCoin } = params;
		...
		const mysoCoinInput =
			...
			coinWithBalance({
				type: mysoCoinType,
				balance: POOL_CREATION_FEE_MYUSD,
			});

		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::create_permissionless_pool`,
			arguments: [
	...
	poolTradeParams = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::pool_trade_params`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	poolBookParams = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::pool_book_params`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	account = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::account`,
	...
	lockedBalance = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::locked_balance`,
			arguments: [tx.object(pool.address), tx.object(managerId)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	 ...
	 * @returns A function that takes a Transaction object
	 */
	getPoolMySoPrice = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);

		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_order_myso_price`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getBalanceManagerIds = (owner: string) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::get_balance_manager_ids`,
			arguments: [tx.object(this.#config.REGISTRY_ID), tx.pure.address(owner)],
		});
	};

	/**
	 * @description Get the balances for a referral (OrderbookPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (OrderbookPoolReferral) to get the balances for
	 * @returns A function that takes a Transaction object
	 */
	getPoolReferralBalances = (poolKey: string, referral: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_pool_referral_balances`,
			arguments: [tx.object(pool.address), tx.object(referral)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	/**
	 * @description Get the multiplier for a referral (OrderbookPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (OrderbookPoolReferral) to get the multiplier for
	 * @returns A function that takes a Transaction object
	 */
	poolReferralMultiplier = (poolKey: string, referral: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::pool_referral_multiplier`,
			arguments: [tx.object(pool.address), tx.object(referral)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	stablePool = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::stable_pool`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	registeredPool = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::registered_pool`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getQuoteQuantityOutInputFee = (poolKey: string, baseQuantity: number) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_quote_quantity_out_input_fee`,
			arguments: [
				tx.object(pool.address),
				tx.pure.u64(Math.round(baseQuantity * baseCoin.scalar)),
				tx.object.clock(),
	...
	getBaseQuantityOutInputFee = (poolKey: string, quoteQuantity: number) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_base_quantity_out_input_fee`,
			arguments: [
				tx.object(pool.address),
	...
	getQuantityOutInputFee =
		(poolKey: string, baseQuantity: number, quoteQuantity: number) => (tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_quantity_out_input_fee`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(Math.round(baseQuantity * baseCoin.scalar)),
					tx.pure.u64(Math.round(quoteQuantity * quoteCoin.scalar)),
	 ...
	 */
	getBaseQuantityIn =
		(poolKey: string, targetQuoteQuantity: number, payWithMySo: boolean) => (tx: Transaction) => {
			const pool = this.#config.getPool(poolKey);
			const baseCoin = this.#config.getCoin(pool.baseCoin);
			const quoteCoin = this.#config.getCoin(pool.quoteCoin);

			return tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_base_quantity_in`,
				arguments: [
					tx.object(pool.address),
	 ...
	 */
	getQuoteQuantityIn =
		(poolKey: string, targetBaseQuantity: number, payWithMySo: boolean) => (tx: Transaction) => {
			const pool = this.#config.getPool(poolKey);
			const baseCoin = this.#config.getCoin(pool.baseCoin);
			const quoteCoin = this.#config.getCoin(pool.quoteCoin);

			return tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_quote_quantity_in`,
				arguments: [
					tx.object(pool.address),
	...
	getAccountOrderDetails = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_account_order_details`,
			arguments: [tx.object(pool.address), tx.object(manager.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getOrderMySoRequired =
		(poolKey: string, baseQuantity: number, price: number) => (tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::get_order_myso_required`,
	...
	accountExists = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::account_exists`,
			arguments: [tx.object(pool.address), tx.object(manager.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	poolTradeParamsNext = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::pool_trade_params_next`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	quorum = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::quorum`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	poolId = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::id`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	canPlaceLimitOrder = (params: CanPlaceLimitOrderParams) => (tx: Transaction) => {
		const { poolKey, balanceManagerKey, price, quantity, isBid, payWithMySo, expireTimestamp } =
			params;
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::can_place_limit_order`,
			arguments: [
				tx.object(pool.address),
	...
	canPlaceMarketOrder = (params: CanPlaceMarketOrderParams) => (tx: Transaction) => {
		const { poolKey, balanceManagerKey, quantity, isBid, payWithMySo } = params;
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::can_place_market_order`,
			arguments: [
				tx.object(pool.address),
				tx.object(manager.address),
				tx.pure.u64(inputQuantity),
	...
	checkMarketOrderParams = (poolKey: string, quantity: number) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::check_market_order_params`,
	...
	checkLimitOrderParams =
		...
		(tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::check_limit_order_params`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(inputPrice),
					tx.pure.u64(inputQuantity),
