...
/**
 * OrderbookAdminContract class for managing admin actions.
 */
export class OrderbookAdminContract {
	#config: OrderbookConfig;

	/**
	 * @param {OrderbookConfig} config Configuration for OrderbookAdminContract
	 */
	...
	createPoolAdmin = (params: CreatePoolAdminParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::create_pool_admin`,
			arguments: [
	...
	unregisterPoolAdmin = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::unregister_pool_admin`,
			arguments: [
	...
	updateAllowedVersions = (poolKey: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::update_allowed_versions`,
			arguments: [
				tx.object(pool.address),
				tx.object(this.#config.REGISTRY_ID),
				tx.object(this.#adminCap()),
	...
	enableVersion = (version: number) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::enable_version`,
			arguments: [
				tx.object(this.#config.REGISTRY_ID),
				tx.pure.u64(version),
				tx.object(this.#adminCap()),
	...
	disableVersion = (version: number) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::disable_version`,
			arguments: [
				tx.object(this.#config.REGISTRY_ID),
				tx.pure.u64(version),
				tx.object(this.#adminCap()),
	...
	setTreasuryAddress = (treasuryAddress: string) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::set_treasury_address`,
			arguments: [
				tx.object(this.#config.REGISTRY_ID),
	...
	addStableCoin = (stableCoinKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::add_stablecoin`,
	...
	removeStableCoin = (stableCoinKey: string) => (tx: Transaction) => {
		const stableCoinType = this.#config.getCoin(stableCoinKey).type;
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::remove_stablecoin`,
			arguments: [tx.object(this.#config.REGISTRY_ID), tx.object(this.#adminCap())],
			typeArguments: [stableCoinType],
		});
	};
	...
	adjustTickSize = (poolKey: string, newTickSize: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::adjust_tick_size_admin`,
			arguments: [
	...
	adjustMinLotSize =
		(poolKey: string, newLotSize: number, newMinSize: number) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::adjust_min_lot_size_admin`,
				arguments: [
					tx.object(pool.address), // pool address
	...
	initBalanceManagerMap = () => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::init_balance_manager_map`,
			arguments: [tx.object(this.#config.REGISTRY_ID), tx.object(this.#adminCap())],
		});
	};
	...
	setEwmaParams = (poolKey: string, params: SetEwmaParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::set_ewma_params`,
			arguments: [
				tx.object(pool.address),
				tx.object(this.#adminCap()),
	...
	enableEwmaState = (poolKey: string, enable: boolean) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::pool::enable_ewma_state`,
			arguments: [
				tx.object(pool.address),
				tx.object(this.#adminCap()),
				tx.pure.bool(enable),
	...
	/**
	 * @description Authorize the MarginApp to access protected features of Orderbook
	 * @returns A function that takes a Transaction object
	 */
	authorizeMarginApp = () => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::authorize_app`,
			arguments: [tx.object(this.#config.REGISTRY_ID), tx.object(this.#adminCap())],
	...
	deauthorizeMarginApp = () => (tx: Transaction) => {
		return tx.moveCall({
			target: `${this.#config.ORDERBOOK_PACKAGE_ID}::registry::deauthorize_app`,
			arguments: [tx.object(this.#config.REGISTRY_ID), tx.object(this.#adminCap())],
			typeArguments: [`${this.#config.MARGIN_PACKAGE_ID}::margin_manager::MarginApp`],
		});
	};
