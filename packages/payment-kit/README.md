...
```bash
npm install --save @socialproof/payment-kit @socialproof/myso
```

## Setup

In order to use the Payment Kit SDK you will first need to create an instance of MySoGrpcClient from
the Typescript SDK, and a client instance of the Payment Kit SDK.

```ts
import { MySoGrpcClient } from '@socialproof/myso/grpc';
import { paymentKit } from '@socialproof/payment-kit';

// Create a MySo client with a Payment Kit extension
...
[myso-payment-kit](https://github.com/MystenLabs/myso-payment-kit) Move Package. This includes:
