...
describe('TransactionAnalyzer - Coin Value Rule', () => {
	...
	it('should calculate coin values for transactions with known prices', async () => {
		...
		const mysoCoin = tx.object(TEST_COIN_1_ID); // 5 MYSO
		...
		// Transfer 1 MYSO, 100 USDC, and 0.5 WETH
		const [mysoSplit] = tx.splitCoins(mysoCoin, [1000000000n]); // 1 MYSO
		...
		tx.transferObjects([mysoSplit, usdcSplit, wethSplit], tx.pure.address('0x456'));
	...
	it('should handle coins without known prices', async () => {
		...
		// Transfer some MYSO (known price) and unknown token (no price)
		const mysoCoin = tx.object(TEST_COIN_1_ID);
		const unknownCoin = tx.object('0xa5c020');

		const [mysoSplit] = tx.splitCoins(mysoCoin, [500000000n]); // 0.5 MYSO
		tx.transferObjects([mysoSplit, unknownCoin], tx.pure.address('0x456'));
	...
	it('should calculate correct USD values with proper decimals', async () => {
		...
		// Split exactly 2.5 MYSO (2500000000 = 2.5 * 10^9)
		// At $2.50 per MYSO, this should be $6.25 total
		const [mysoSplit] = tx.splitCoins(mysoCoin, [2500000000n]);
		tx.transferObjects([mysoSplit], tx.pure.address('0x456'));
	...
	it('should handle mixed scenarios with some priced and some unpriced coins', async () => {
		...
		// Use both types
		const mysoCoin = tx.object(TEST_COIN_1_ID); // Has price
		const usdcCoin = tx.object(TEST_USDC_COIN_ID); // Has price
		const unknownCoin = tx.object('0xa5c021'); // No price

		const [mysoSplit] = tx.splitCoins(mysoCoin, [1000000000n]); // 1 MYSO = $2.50
		const [usdcSplit] = tx.splitCoins(usdcCoin, [50000000n]); // 50 USDC = $50.00

		tx.transferObjects([mysoSplit, usdcSplit, unknownCoin], tx.pure.address('0x456'));
	...
	it('should handle price provider errors gracefully', async () => {
		...
		const mysoCoin = tx.object(TEST_COIN_1_ID);
		const [mysoSplit] = tx.splitCoins(mysoCoin, [1000000000n]);
		tx.transferObjects([mysoSplit], tx.pure.address('0x456'));
	...
	it('should only calculate values for outflow coins (spent coins)', async () => {
		...
		const [mysoSplit] = tx.splitCoins(mysoCoin, [1000000000n]);
		tx.transferObjects([mysoSplit], tx.pure.address(DEFAULT_SENDER));
