...
describe('TransactionAnalyzer - Inputs Rule', () => {
	it('should analyze all input types in a single transaction', async () => {
		...
		const optionValue = tx.pure(bcs.option(bcs.Address).serialize(normalizeMySoAddress('0x123')));
		...
		if (addressInput?.$kind === 'Pure') {
			expect(bcs.Address.fromBase64(addressInput.bytes)).toBe(normalizeMySoAddress('0x456'));
		}
		...
		if (ownedObjectInput?.$kind === 'Object') {
			expect(ownedObjectInput.object.objectId).toBe(normalizeMySoAddress(TEST_COIN_1_ID));
		...
		if (sharedObjectInput?.$kind === 'Object') {
			expect(sharedObjectInput.object.objectId).toBe(normalizeMySoAddress(TEST_SHARED_OBJECT_ID));
		...
		if (receivingObjectInput?.$kind === 'Object') {
			expect(receivingObjectInput.object.objectId).toBe(normalizeMySoAddress(TEST_NFT_ID));
