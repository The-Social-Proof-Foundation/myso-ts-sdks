...
import type { ClientWithCoreApi, MySoClientRegistration } from '@socialproof/myso/client';
...
export function zksend<Name extends string = 'zksend'>({
	name = 'zksend' as Name,
	...options
}: ZkSendOptions<Name> = {}): MySoClientRegistration<ZkSendCompatibleClient, Name, ZkSendClient> {
	return {
		name,
		register: (client) => {
