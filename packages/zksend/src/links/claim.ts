...
export class ZkSendLink {
	...
	claimed?: boolean;
	claimedBy?: string;
	bagObject?: MySoClientTypes.GetDynamicFieldResponse['dynamicField'] | null;
	...
	constructor({
		...
		this.address = address ?? keypair!.toMySoAddress();
	...
	async loadAssets(
		options: {
			transaction?: MySoClientTypes.TransactionResult;
			loadAssets?: boolean;
	...
	async claimAssets(
		...
		const sponsored = await this.#createSponsoredTransaction(
			tx,
			address,
			reclaim ? address : this.keypair!.toMySoAddress(),
		);
	...
	createClaimTransaction(
		...
		const sender = reclaim ? address : this.keypair!.toMySoAddress();
		tx.setSender(sender);
	...
	async createRegenerateTransaction(
		...
		const to = tx.pure.address(newLinkKp.toMySoAddress());
	...
	async #loadBag({
		...
		objectsResponse.objects.forEach((object, i) => {
			...
			if (
				type.address === normalizeMySoAddress('0x2') &&
				type.module === 'coin' &&
				type.name === 'Coin'
			) {
